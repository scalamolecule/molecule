...
case class Transactions_async(
  suite: MUnitSuite,
  api: Api_async_transact with Api_async with Spi_async with DbProviders
) extends TestUtils {
    ...
    for {
      _ <- unitOfWork {
        for {
          ...
          _ <- Entity.int.query.get.map(_ ==> List(2, 3))
    ...
    for {
      ...
      _ <- unitOfWork {
        for {
          ...
          _ <- Entity.int.query.get.map(_ ==> List(1, 2, 3, 4))

          _ <- Entity(2).delete.transact
          _ <- Entity.int.query.get.map(_ ==> List(1, 3, 4))

          _ <- Entity(4).int.*(10).update.transact
          _ <- Entity.int.query.get.map(_ ==> List(1, 3, 40))
