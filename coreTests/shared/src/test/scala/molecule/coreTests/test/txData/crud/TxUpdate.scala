...
trait TxUpdate extends CoreTestSuite with ApiAsyncImplicits { self: SpiAsync  =>
  ...
  override lazy val tests = Tests {
    ...
    "Update base and tx data" - types { implicit conn =>
      for {
    ...
    "Update base data only" - types { implicit conn =>
      for {
        ...
        // Note that the initial tx data is still tied to the first transaction entity
        _ <- Other.eid(tx).s.query.get.map(_ ==> List((tx, "meta")))

        // OBS: The old tx data is no longer associated with the updated base entity.
        // Transaction data is tied only to the transaction entity where it was created!
        _ <- Ns.int.Tx(Other.s).query.get.map(_ ==> Nil)

        // If we instead update both the base value(s) and tx data we'll get the expected result
    ...
    "Update tx data only" - types { implicit conn =>
      for {
        // Initial entity with tx data
        txReport <- Ns.int(1).Tx(Other.s("meta")).save.transact
        eid = txReport.eid
        ...
        // Update tx data only by using the tx id
