...
trait ManyOwned_Seq_remove extends CoreTestSuite with ApiAsync { spi: SpiAsync =>
  ...
  override lazy val tests = Tests {
    ...
    "id-filter - ref - value" - refs { implicit conn =>
      for {
        ...
        _ <- A(a, b, c, d, e, f).OwnBb.iSeq.remove(4, 5).update.transact

        _ <- A.i.a1.OwnBb.*?(B.s_?.iSeq_?).query.get.map(_ ==> List(
          (1, List()),
          (2, List((Some("a"), None))),
          (3, List((Some("b"), None), (Some("c"), None))),
          (4, List((Some("d"), Some(Seq(1, 2, 1))))),
    ...
    "filter - ref - value" - refs { implicit conn =>
      for {
        ...
        // Filter by A ids, update B values
        _ <- A.i_.OwnBb.iSeq.remove(4, 5).update.transact
    ...
    "value - ref - filter" - refs { implicit conn =>
      for {
        ...
        _ <- A.iSeq_?.OwnBb.*(B.s.a1).query.get.map(_.sortBy(_._2.headOption.toString) ==> List(
        ...
        _ <- A.iSeq.ownBb_().query.get.map(_ ==> List(Seq(1, 2, 3)))
      } yield ()
    }
    ...
    "ref - filter/value" - refs { implicit conn =>
      for {
        ...
        _ <- A.OwnBb.s_.iSeq.remove(3, 4).update.transact
