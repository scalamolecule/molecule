...
trait FilterSeq_UUID_ extends CoreTestSuite with Api_async { spi: Spi_async =>
  ...
  override lazy val tests = Tests {
    ...
    "Mandatory" - {
      ...
      "has" - types { implicit conn =>
        for {
          _ <- Ns.i.uuidSeq.insert(a, b).transact
          ...
          // "Has this OR that"
          _ <- Ns.i.a1.uuidSeq.has(uuid0, uuid1).query.get.map(_ ==> List(a))
          ...
          // Same as
          _ <- Ns.i.a1.uuidSeq.has(List(uuid0, uuid1)).query.get.map(_ ==> List(a))
    ...
    "Tacit" - {
      ...
      "has" - types { implicit conn =>
        for {
          ...
          // "Has this OR that"
          _ <- Ns.i.a1.uuidSeq_.has(uuid0, uuid1).query.get.map(_ ==> List(1))
          ...
          _ <- Ns.i.a1.uuidSeq_.has(uuid3, uuid4).query.get.map(_ ==> List(2))
          // Same as
          _ <- Ns.i.a1.uuidSeq_.has(List(uuid0, uuid1)).query.get.map(_ ==> List(1))
          ...
          _ <- Ns.i.a1.uuidSeq_.has(List(uuid3, uuid4)).query.get.map(_ ==> List(2))
