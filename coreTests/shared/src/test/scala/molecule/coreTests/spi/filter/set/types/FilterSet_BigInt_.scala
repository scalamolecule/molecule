...
trait FilterSet_BigInt_ extends CoreTestSuite with Api_async { spi: Spi_async =>
  ...
  override lazy val tests = Tests {
    ...
    "Mandatory" - {
      ...
      "has" - types { implicit conn =>
        for {
          _ <- Ns.i.bigIntSet.insert(a, b).transact
    ...
    "Tacit" - {
      ...
      "has" - types { implicit conn =>
        for {
          ...
          // Same as
          _ <- Ns.i.a1.bigIntSet_.has(Seq(bigInt0, bigInt1)).query.get.map(_ ==> List(1))
          ...
          _ <- Ns.i.a1.bigIntSet_.has(Seq(bigInt3, bigInt4)).query.get.map(_ ==> List(2))
