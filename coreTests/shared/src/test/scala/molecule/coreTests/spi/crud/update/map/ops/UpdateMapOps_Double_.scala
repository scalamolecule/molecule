// GENERATED CODE ********************************
package molecule.coreTests.spi.crud.update.map.ops

import molecule.core.api.ApiAsync
import molecule.core.spi.SpiAsync
import molecule.core.util.Executor._
import molecule.coreTests.async._
import molecule.coreTests.dataModels.core.dsl.Types._
import molecule.coreTests.setup.CoreTestSuite
import utest._

trait UpdateMapOps_Double_ extends CoreTestSuite with ApiAsync { spi: SpiAsync =>

  override lazy val tests = Tests {

    "apply (replace/add all)" - types { implicit conn =>
      for {
        id <- Ns.i(42).save.transact.map(_.id)
        // Map attribute not yet asserted
        _ <- Ns.doubleMap.query.get.map(_ ==> Nil)

        // Applying Map of pairs to non-asserted Map attribute adds the attribute with the update
        _ <- Ns(id).doubleMap(Map(pdouble1, pdouble2)).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2))

        // Applying Map of pairs replaces previous Map
        _ <- Ns(id).doubleMap(Map(pdouble2, pdouble3)).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble2, pdouble3))

        // Add other attribute and update Map attribute in one go
        _ <- Ns(id).s("foo").doubleMap(Map(pdouble3, pdouble4)).update.transact
        _ <- Ns.i.s.doubleMap.query.get.map(_.head ==> (42, "foo", Map(pdouble3, pdouble4)))

        // Applying empty Map of pairs deletes map
        _ <- Ns(id).doubleMap(Map.empty[String, Double]).update.transact
        _ <- Ns.doubleMap.query.get.map(_ ==> Nil)

        _ <- Ns(id).doubleMap(Map(pdouble1, pdouble2)).update.transact
        // Apply nothing to delete attribute
        _ <- Ns(id).doubleMap().update.transact
        _ <- Ns.doubleMap.query.get.map(_ ==> Nil)

        // Entity still has other attributes
        _ <- Ns.i.s.query.get.map(_.head ==> (42, "foo"))
      } yield ()
    }


    "add" - types { implicit conn =>
      for {
        id <- Ns.i(42).save.transact.map(_.id)
        // Map attribute not yet asserted
        _ <- Ns.doubleMap.query.get.map(_ ==> Nil)

        // Adding value to non-asserted Map attribute adds the attribute with the update
        _ <- Ns(id).doubleMap.add("a" -> double0).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map("a" -> double0))

        // Adding existing pair to Map changes nothing
        _ <- Ns(id).doubleMap.add("a" -> double0).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map("a" -> double0))

        // Adding pair with existing key replaces the value of the pair
        _ <- Ns(id).doubleMap.add("a" -> double1).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1)) // "a" -> double1

        // Add new pair
        _ <- Ns(id).doubleMap.add(pdouble2).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2))

        // Add multiple pairs with varargs
        _ <- Ns(id).doubleMap.add(pdouble3, pdouble4).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2, pdouble3, pdouble4))

        // Add multiple pairs with Iterable
        _ <- Ns(id).doubleMap.add(List(pdouble5, pdouble6)).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2, pdouble3, pdouble4, pdouble5, pdouble6))

        // Adding empty Iterable of pairs has no effect
        _ <- Ns(id).doubleMap.add(Vector.empty[(String, Double)]).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2, pdouble3, pdouble4, pdouble5, pdouble6))
      } yield ()
    }


    "remove" - types { implicit conn =>
      for {
        id <- Ns.i(42).save.transact.map(_.id)
        // Map attribute not yet asserted
        _ <- Ns.doubleMap.query.get.map(_ ==> Nil)

        // Removing pair by key from non-asserted Map has no effect
        _ <- Ns(id).doubleMap.remove(string1).update.transact
        _ <- Ns.doubleMap.query.get.map(_ ==> Nil)

        // Start with some pairs
        _ <- Ns(id).doubleMap.add(pdouble1, pdouble2, pdouble3, pdouble4, pdouble5, pdouble6, pdouble7).update.transact

        // Remove pair by String key
        _ <- Ns(id).doubleMap.remove(string7).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2, pdouble3, pdouble4, pdouble5, pdouble6))

        // Removing non-existing key has no effect
        _ <- Ns(id).doubleMap.remove(string9).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2, pdouble3, pdouble4, pdouble5, pdouble6))

        // Removing duplicate keys removes only pairs with the distinct key (distinct key value semantics of Map)
        _ <- Ns(id).doubleMap.remove(string6, string6).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2, pdouble3, pdouble4, pdouble5))

        // Remove multiple pairs by varargs of keys
        _ <- Ns(id).doubleMap.remove(string4, string5).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1, pdouble2, pdouble3))

        // Remove multiple pairs by Seq of keys (not Iterable)
        _ <- Ns(id).doubleMap.remove(List(string2, string3)).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1))

        // Removing pairs with empty Seq of keys has no effect
        _ <- Ns(id).doubleMap.remove(Seq.empty[String]).update.transact
        _ <- Ns.doubleMap.query.get.map(_.head ==> Map(pdouble1))

        // Removing all remaining pairs deletes the attribute
        _ <- Ns(id).doubleMap.remove(Seq(string1)).update.transact
        _ <- Ns.doubleMap.query.get.map(_ ==> Nil)
      } yield ()
    }
  }
}
