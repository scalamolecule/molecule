...
trait ManyOwned_Map_remove extends CoreTestSuite with ApiAsync { spi: SpiAsync =>
  ...
  override lazy val tests = Tests {
    ...
    "id-filter - ref - value" - refs { implicit conn =>
      for {
        ...
        _ <- A(a, b, c, d, e, f).OwnBb.iMap.remove(string4, string5).update.transact

        // 3 nested entities updated
        _ <- A.i.a1.OwnBb.*?(B.s_?.iMap_?).query.get.map(_ ==> List(
          (1, List()),
          (2, List((Some("a"), None))),
          (3, List((Some("b"), None), (Some("c"), None))),
          (4, List((Some("d"), Some(Map(pint1, pint2))))),
    ...
    "filter - ref - value" - refs { implicit conn =>
      for {
        ...
        // Filter by A ids, update B values
        _ <- A.i_.OwnBb.iMap.remove(string4, string5).update.transact
    ...
    "value - ref - filter" - refs { implicit conn =>
      for {
        ...
        _ <- A.iMap_?.OwnBb.*(B.s.a1).query.get.map(_.sortBy(_._2.headOption.toString) ==> List(
        ...
        _ <- A.iMap.ownBb_().query.get.map(_ ==> List(Map(pint1, pint3)))
      } yield ()
    }
    ...
    "ref - filter/value" - refs { implicit conn =>
      for {
        ...
        _ <- A.OwnBb.s_.iMap.remove(string3, string4).update.transact
