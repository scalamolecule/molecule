...
trait AggrOne_LocalDateTime_ extends CoreTestSuite with ApiAsync { spi: SpiAsync =>
  ...
  override lazy val tests = Tests {
    ...
    "rand" - types { implicit conn =>
      for {
        ...
        _ <- Ns.localDateTime(rand).query.get.map(res => all.contains(res.head) ==> true)
        _ <- Ns.localDateTime(rand(1)).query.get.map(res => all.intersect(res.head).nonEmpty ==> true)
        _ <- Ns.localDateTime(rand(2)).query.get.map(res => all.intersect(res.head).nonEmpty ==> true)
      } yield ()
    }
