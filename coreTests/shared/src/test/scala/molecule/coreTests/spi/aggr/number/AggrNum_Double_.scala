...
trait AggrNum_Double_ extends CoreTestSuite with ApiAsync { spi: SpiAsync =>
      ...
      database match {
        ...
        case "MongoDB" =>
          for {
            ...
            _ <- Ns.double(median).query.get.map(_.head ==~ double2.toString.toDouble) // whole middle number
