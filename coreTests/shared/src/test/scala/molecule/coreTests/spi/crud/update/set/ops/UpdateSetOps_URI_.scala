  ...
  override lazy val tests = Tests {
    ...
    "apply (replace/add all)" - types { implicit conn =>
      for {
        ...
        _ <- Ns(id).uris(Set.empty[URI]).update.transact
    ...
    "add" - types { implicit conn =>
      for {
        ...
        _ <- Ns(id).uris.add(Seq(uri5, uri6)).update.transact
    ...
    "remove" - types { implicit conn =>
      for {
        id <- Ns.uris(Set(uri1, uri2, uri3, uri4, uri5, uri6, uri7)).save.transact.map(_.id)
        ...
        _ <- Ns(id).uris.remove(uri9).update.transact
        _ <- Ns.uris.query.get.map(_.head ==> Set(uri1, uri2, uri3, uri4, uri5, uri6))

        // Removing duplicate values removes the distinct value
        _ <- Ns(id).uris.remove(uri6, uri6).update.transact
        _ <- Ns.uris.query.get.map(_.head ==> Set(uri1, uri2, uri3, uri4, uri5))

        // Remove multiple values (vararg)
        _ <- Ns(id).uris.remove(uri4, uri5).update.transact
        _ <- Ns.uris.query.get.map(_.head ==> Set(uri1, uri2, uri3))

        // Remove multiple values (Seq)
        _ <- Ns(id).uris.remove(Seq(uri2, uri3)).update.transact
