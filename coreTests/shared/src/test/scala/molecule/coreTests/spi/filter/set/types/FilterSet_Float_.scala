...
trait FilterSet_Float_ extends CoreTestSuite with Api_async { spi: Spi_async =>
  ...
  override lazy val tests = Tests {
    ...
    "Mandatory" - {
      ...
      "has" - types { implicit conn =>
        for {
          _ <- Ns.i.floatSet.insert(a, b).transact
    ...
    "Tacit" - {
      ...
      "has" - types { implicit conn =>
        for {
          ...
          // Same as
          _ <- Ns.i.a1.floatSet_.has(Seq(float0, float1)).query.get.map(_ ==> List(1))
          ...
          _ <- Ns.i.a1.floatSet_.has(Seq(float3, float4)).query.get.map(_ ==> List(2))
