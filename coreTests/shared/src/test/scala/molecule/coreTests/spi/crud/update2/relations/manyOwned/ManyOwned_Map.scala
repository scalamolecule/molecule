...
trait ManyOwned_Map extends CoreTestSuite with ApiAsync { spi: SpiAsync =>
  ...
  override lazy val tests = Tests {
    ...
    "id-filter - ref - value" - refs { implicit conn =>
      for {
        ...
        _ <- A(a, b, c, d, e, f).OwnBb.iMap(Map(pint4, pint5)).update.transact
    ...
    "filter - ref - value" - refs { implicit conn =>
      for {
        ...
        // Filter by A ids, update B values
        _ <- A.i_.OwnBb.iMap(Map(pint4, pint5)).update.transact
    ...
    "value - ref - filter" - refs { implicit conn =>
      for {
        ...
        _ <- A.iMap(Map(pint3, pint4)).OwnBb.s_.update.transact
    ...
    "ref - filter/value" - refs { implicit conn =>
      for {
        ...
        _ <- A.OwnBb.s_.iMap(Map(pint3, pint4)).update.transact
    ...
    "backref" - refs { implicit conn =>
      for {
        id <- A.iMap(Map(pint1)).OwnBb.iMap(Map(pint2))._A.Cc.iMap(Map(pint3)).save.transact.map(_.id)
        _ <- A.iMap.OwnBb.iMap._A.Cc.iMap.query.get.map(_ ==> List((Map(pint1), Map(pint2), Map(pint3))))

        // Updating A.OwnBb.iMap and A.Cc.iMap
        _ <- A(id).iMap(Map(pint1)).OwnBb.iMap(Map(pint1))._A.Cc.iMap(Map(pint1)).update.transact
        _ <- A.iMap.OwnBb.iMap._A.Cc.iMap.query.get.map(_ ==> List((Map(pint1), Map(pint1), Map(pint1))))
      } yield ()
    }
  }
}
