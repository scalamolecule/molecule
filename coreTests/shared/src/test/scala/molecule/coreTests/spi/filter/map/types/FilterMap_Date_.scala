...
trait FilterMap_Date_ extends CoreTestSuite with ApiAsync { spi: SpiAsync =>
    ...
    "Mandatory" - {
      ...
      "Get value by key" - types { implicit conn =>
        ...
        for {
          ...
          _ <- Ns.i.a1.dateMap("-").query.get.map(_ ==> Nil) // When no map is saved
    ...
    "Optional" - {
      ...
      "Get optional value by key" - types { implicit conn =>
        ...
        for {
          ...
          _ <- Ns.i.a1.dateMap_?("-").query.get.map(_ ==> List((1, None), (2, None), (3, None)))
    ...
    "Tacit" - {
      ...
      "contains key(s)" - types { implicit conn =>
        for {
          ...
          _ <- Ns.i.a1.dateMap.dateMap_("-").query.get.map(_ ==> Nil)
      ...
      "doesn't contain key(s)" - types { implicit conn =>
        for {
          ...
          // Combine with retrieval
          _ <- Ns.i.a1.dateMap.dateMap_.not("-").query.get.map(_ ==> List((1, Map(a1, b2)), (2, Map(b3, c4))))
