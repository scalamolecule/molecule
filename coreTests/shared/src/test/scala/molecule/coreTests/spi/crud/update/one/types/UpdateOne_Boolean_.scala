...
trait UpdateOne_Boolean_ extends CoreTestSuite with ApiAsync { spi: SpiAsync =>
  ...
  override lazy val tests = Tests {
    ...
    "Semantics" - types { implicit conn =>
      for {
        ...
        // Update value
        _ <- Ns(id).boolean(boolean2).update.transact
        _ <- Ns.boolean.query.get.map(_.head ==> boolean2)

        // Add new attribute and update value in one go
        _ <- Ns(id).s("foo").boolean(boolean3).update.transact
        _ <- Ns.i.s.boolean.query.get.map(_.head ==> (42, "foo", boolean3))

        // Apply nothing to delete value
        _ <- Ns(id).boolean().update.transact
        _ <- Ns.boolean.query.get.map(_ ==> Nil)
