          ...
          .map(_ ==> "Unexpected success").recover { case ModelError(err) =>
            err ==>
              """Cardinality-set filter attributes not allowed to do additional filtering. Found:
                |  AttrSetManInt("Ref", "ints", Neq, Seq(Set(2, 3)), None, None, Nil, Nil, None, None, Seq(1, 80))""".stripMargin
          }
        _ <- Ns.int.ints.not(Set(2, 3)).Ref.ints(Ns.ints_).int.query.get
          .map(_ ==> "Unexpected success").recover { case ModelError(err) =>
            err ==>
              """Cardinality-set filter attributes not allowed to do additional filtering. Found:
                |  AttrSetManInt("Ns", "ints", Neq, Seq(Set(2, 3)), None, None, Nil, Nil, None, None, Seq(0, 30))""".stripMargin
          }
