...
trait FilterSet_URI_ extends CoreTestSuite with Api_async { spi: Spi_async =>
  ...
  override lazy val tests = Tests {
    ...
    "Mandatory" - {
      ...
      "has" - types { implicit conn =>
        for {
          _ <- Ns.i.uriSet.insert(a, b).transact
    ...
    "Tacit" - {
      ...
      "has" - types { implicit conn =>
        for {
          ...
          // Same as
          _ <- Ns.i.a1.uriSet_.has(Seq(uri0, uri1)).query.get.map(_ ==> List(1))
          ...
          _ <- Ns.i.a1.uriSet_.has(Seq(uri3, uri4)).query.get.map(_ ==> List(2))
