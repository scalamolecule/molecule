...
trait UpdateOne_UUID_ extends CoreTestSuite with ApiAsync { spi: SpiAsync =>
  ...
  override lazy val tests = Tests {
    ...
    "Semantics" - types { implicit conn =>
      for {
        ...
        // Update value
        _ <- Ns(id).uuid(uuid2).update.transact
        _ <- Ns.uuid.query.get.map(_.head ==> uuid2)

        // Add new attribute and update value in one go
        _ <- Ns(id).s("foo").uuid(uuid3).update.transact
        _ <- Ns.i.s.uuid.query.get.map(_.head ==> (42, "foo", uuid3))

        // Apply nothing to delete value
        _ <- Ns(id).uuid().update.transact
        _ <- Ns.uuid.query.get.map(_ ==> Nil)
