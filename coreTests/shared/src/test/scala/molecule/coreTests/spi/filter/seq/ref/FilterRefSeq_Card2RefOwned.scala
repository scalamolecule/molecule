  ...
  override lazy val tests = Tests {
    ...
    "tacit" - refs { implicit conn =>
      for {
        ...
        _ <- A.i.a1.OwnBb.iSeq_().query.get.map(_ ==> List(4))
      } yield ()
    }
    ...
    "optional" - refs { implicit conn =>
      ...
      for {
        ...
        _ <- A.i.a1.OwnBb.iSeq_?.has(Some(1)).query.get.map(_ ==> List(
          (1, Some(List(1, 2, 2))),
        ))
        _ <- A.i.a1.OwnBb.iSeq_?.has(Some(2)).query.get.map(_ ==> List(
          (1, Some(List(1, 2, 2))),
          (2, Some(List(2))),
        ))
        // same as
        _ <- A.i.a1.OwnBb.iSeq_?.has(Some(List(2))).query.get.map(_ ==> List(
          (1, Some(List(1, 2, 2))),
          (2, Some(List(2))),
        ))
        // has 2 or 3
        _ <- A.i.a1.OwnBb.iSeq_?.has(Some(List(2, 3))).query.get.map(_ ==> List(
          (1, Some(List(1, 2, 2))),
          (2, Some(List(2))),
          (3, Some(List(3))),
        ))
        ...
        // None matches non-asserted values
        _ <- A.i.a1.OwnBb.iSeq_?.has(Option.empty[List[Int]]).query.get.map(_ ==> List((4, None)))

        // Empty Lists match nothing
        _ <- A.i.a1.OwnBb.iSeq_?.has(Some(List.empty[Int])).query.get.map(_ ==> Nil)


        // hasNo

        _ <- A.i.a1.OwnBb.iSeq_?.hasNo(Some(1)).query.get.map(_ ==> List(
          (2, Some(List(2))),
          (2, Some(List(7))),
          (3, Some(List(3))),
        ))
        _ <- A.i.a1.OwnBb.iSeq_?.hasNo(Some(2)).query.get.map(_ ==> List(
          (2, Some(List(7))),
          (3, Some(List(3))),
        ))
        _ <- A.i.a1.OwnBb.iSeq_?.hasNo(Some(3)).query.get.map(_ ==> List(
          (1, Some(List(1, 2, 2))),
          (2, Some(List(2))),
          (2, Some(List(7))),
        ))
        ...
        // Negating None matches all asserted values
        _ <- A.i.a1.OwnBb.iSeq_?.hasNo(Option.empty[List[Int]]).query.get.map(_ ==> allAssertedOptional)

        // Negating empty Lists match nothing
        _ <- A.i.a1.OwnBb.iSeq_?.hasNo(Some(List.empty[Int])).query.get.map(_ ==> allAssertedOptional)
      } yield ()
    }
  }
}
