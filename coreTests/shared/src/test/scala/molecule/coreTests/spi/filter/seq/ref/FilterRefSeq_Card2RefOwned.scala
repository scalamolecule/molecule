...
trait FilterRefSeq_Card2RefOwned extends CoreTestSuite with Api_async { spi: Spi_async =>
  ...
  override lazy val tests = Tests {
    ...
    "tacit" - refs { implicit conn =>
      for {
        ...
        _ <- A.i.a1.OwnBb.iSeq_.has(2, 7).query.get.map(_ ==> List(1, 2))
        _ <- A.i.a1.OwnBb.iSeq_.has(2, 3).query.get.map(_ ==> List(1, 2, 3))

        // hasNo
        _ <- A.i.a1.OwnBb.iSeq_.hasNo(1).query.get.map(_ ==> List(2, 3))
        _ <- A.i.a1.OwnBb.iSeq_.hasNo(2).query.get.map(_ ==> List(2, 3))
        _ <- A.i.a1.OwnBb.iSeq_.hasNo(3).query.get.map(_ ==> List(1, 2))
