...
trait Base[Tpl] extends BaseHelpers with JavaConversions { self: Model2Query[Tpl] =>
  ...
  //  final protected val find         = new ArrayBuffer[Var]
  ...
  final protected var exprAttrVars : Map[String, Var]                = Map.empty[String, Var]
  final protected var exprAttrVars1: Map[String, (Var, Var)]         = Map.empty[String, (Var, Var)]
  final protected var exprAttrVars2: Map[String, (Var, Var) => Unit] = Map.empty[String, (Var, Var) => Unit]
  final protected val expectedExprAttrs                              = mutable.Set.empty[String]
  ...
  def getVar(attr: Attr) = {
    exprAttrVars.getOrElse(attr.name, vv)
  }
