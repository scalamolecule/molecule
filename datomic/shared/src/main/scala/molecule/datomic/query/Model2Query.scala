...
class Model2Query[Tpl](elements: Seq[Element]) {
  ...
  final protected def resolve(es: List[Var], elements: Seq[Element]): List[Var] = elements match {
    case element :: tail => element match {
      case a: AtomMandatory => resolve(atomMan(es, a), tail)
      case a: AtomOptional  => resolve(atomOpt(es, a), tail)
  ...
  private def atomMan(es: List[Var], atom: AtomMandatory): List[Var] = {
    val (e, a) = (es.last, s":${atom.ns}/${atom.attr}")
    atom match {
      case AtomManString(_, _, op, vs, _, _, sort) => atomMan(es, e, a, op, vs)
      case AtomManInt(_, _, op, vs, _, _, sort)    => atomMan(es, e, a, op, vs)
  ...
  private def atomOpt(es: List[Var], a: AtomOptional): List[Var] = a match {
    case _: AtomOptString => es
    case _: AtomOptInt    => es
    case other            => throw MoleculeException("StmtsBuilder.res: Unexpected element: " + other)
  }
