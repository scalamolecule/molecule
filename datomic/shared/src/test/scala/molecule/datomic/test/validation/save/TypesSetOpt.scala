...
object TypesSetOpt extends DatomicTestSuite {
  ...
  override lazy val tests = Tests {
    ...
    "Float" - validation { implicit conn =>
      for {
        _ <- Type.floats_?(Some(Set(0.1f, 1.1f, 2.2f))).save.transact.expect {
          case ValidationErrors(errorMap, _) =>
            errorMap.head._2 ==> Seq(
    ...
    "Double" - validation { implicit conn =>
      for {
        _ <- Type.doubles_?(Some(Set(0.1, 1.1, 2.2))).save.transact.expect {
          case ValidationErrors(errorMap, _) =>
            errorMap.head._2 ==> Seq(
    ...
    "BigInt" - validation { implicit conn =>
      for {
        _ <- Type.bigInts_?(Some(Set(BigInt(0), BigInt(1), BigInt(2)))).save.transact.expect {
          case ValidationErrors(errorMap, _) =>
            errorMap.head._2 ==> Seq(
    ...
    "BigDecimal" - validation { implicit conn =>
      for {
        _ <- Type.bigDecimals_?(Some(Set(BigDecimal(0.1), BigDecimal(1.1), BigDecimal(2.2)))).save.transact.expect {
          case ValidationErrors(errorMap, _) =>
            errorMap.head._2 ==> Seq(
