...
object StarwarsTest extends ZIOSpecDefault with StarwarsServer {
  ...
  def graphql(query: String, expected: String): ZIO[Any, CalibanError.ValidationError, TestResult] = {
    val interpreter = (graphQL(starwarsResolver) @@ maxDepth(5)).interpreter
  ...
  override def spec =
    suite("Molecule ZIO api")(
      test("hero (arg is None)") {
        ...
        //        hero.name.query.get.map(_ ==> ("Droid", "R2-D2"))
      },
