  ...
  override def spec =
    suite("ClientWriterSpec")(
      test("simple object type") {
        ...
        val hej = Parser.parseQuery(schema).flatMap { case doc@Document(x, y) =>
          ...
          val xx = doc.definitions.collect {
            case ObjectTypeDefinition(a, name, b, c, d)    =>
              ...
              "1 " + name
            case InputObjectTypeDefinition(_, name, _, _)  => "2 " + name
            case EnumTypeDefinition(_, name, _, _)         => "3 " + name
            case UnionTypeDefinition(_, name, _, _)        => "4 " + name
            case ScalarTypeDefinition(_, name, _)          => "5 " + name
            case InterfaceTypeDefinition(_, name, _, _, _) => "6 " + name
          }
