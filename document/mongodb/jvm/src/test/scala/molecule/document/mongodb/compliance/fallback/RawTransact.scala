...
object RawTransact extends TestSuite_mongodb {
  ...
  override lazy val tests = Tests {
    ...
    "Insert Set" - types { implicit conn =>
      for {
        _ <- insert("stringSet", s"""["$string1", "$string2"]""")
        _ <- insert("intSet", s"""[$int1, $int2]""")
        _ <- insert("longSet", s"""[$long1, $long2]""")
        _ <- insert("floatSet", s"""[$float1, $float2]""")
        _ <- insert("doubleSet", s"""[$double1, $double2]""")
        _ <- insert("booleanSet", s"""[$boolean1, $boolean2]""")
        _ <- insert("bigIntSet", s"""["$bigInt1", "$bigInt2"]""")
        _ <- insert("bigDecimalSet", s"""["$bigDecimal1", "$bigDecimal2"]""")
        _ <- insert("dateSet", s"""[${date1.getTime}, ${date2.getTime}]""")
        _ <- insert("durationSet", s"""["$duration1", "$duration2"]""")
        _ <- insert("instantSet", s"""["$instant1", "$instant2"]""")
        _ <- insert("localDateSet", s"""["$localDate1", "$localDate2"]""")
        _ <- insert("localTimeSet", s"""["$localTime1", "$localTime2"]""")
        _ <- insert("localDateTimeSet", s"""["$localDateTime1", "$localDateTime2"]""")
        _ <- insert("offsetTimeSet", s"""["$offsetTime1", "$offsetTime2"]""")
        _ <- insert("offsetDateTimeSet", s"""["$offsetDateTime1", "$offsetDateTime2"]""")
        _ <- insert("zonedDateTimeSet", s"""["$zonedDateTime1", "$zonedDateTime2"]""")
        _ <- insert("uuidSet", s"""["$uuid1", "$uuid2"]""")
        _ <- insert("uriSet", s"""["$uri1", "$uri2"]""")
        _ <- insert("byteSet", s"""[$byte1, $byte2]""")
        _ <- insert("shortSet", s"""[$short1, $short2]""")
        _ <- insert("charSet", s"""["$char1", "$char2"]""")
