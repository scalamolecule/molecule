...
import molecule.document.mongodb.schema._
import molecule.document.mongodb.facade.MongoConn_JVM
import org.testcontainers.containers.MongoDBContainer
import org.testcontainers.utility.DockerImageName

object Connection_mongodb2 {
  ...
  val conn_Types      = MongoConn_JVM(proxy(Types2Schema), mongoClient, dbName, mongoDb)
  val conn_Refs       = MongoConn_JVM(proxy(Refs2Schema), mongoClient, dbName, mongoDb)
  val conn_Uniques    = MongoConn_JVM(proxy(Uniques2Schema), mongoClient, dbName, mongoDb)
  val conn_Validation = MongoConn_JVM(proxy(Validation2Schema), mongoClient, dbName, mongoDb)
}
