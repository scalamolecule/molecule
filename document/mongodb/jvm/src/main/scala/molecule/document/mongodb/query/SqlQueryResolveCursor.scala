...
case class SqlQueryResolveCursor[Tpl](
  ...
  def getListFromCursor_sync(implicit conn: MongoDBConn_JVM)
  ...
  private def getInitialPage(limit: Int)(implicit conn: MongoDBConn_JVM)
  : (List[Tpl], String, Boolean) = {
    val forward      = limit > 0
  ...
  private def initialCursor(conn: MongoDBConn_JVM, tpls: List[Tpl]): String = {
