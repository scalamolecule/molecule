...
trait ResolveExprOne extends ResolveExpr with LambdasOne { self: MongoQueryBase =>
  ...
  override protected def resolveAttrOneMan(attr: AttrOneMan): Unit = {
    aritiesAttr()
    attr match {
      case at: AttrOneManID             =>
        if (at.attr == "id")
          idField = true
        man(attr, at.vs, resString)
  ...
  protected def man[T: ClassTag](attr: Attr, args: Seq[T], res: Res[T]): Unit = {
    val field = attr.attr
    //    select += col
  ...
  protected def tac[T: ClassTag](attr: Attr, args: Seq[T], res: Res[T]): Unit = {
  ...
  //  protected def aggr[T: ClassTag](col: String, fn: String, optN: Option[Int], res: Res[T]): Unit = {
  ...
  protected def equal[T: ClassTag](field: String, args: Seq[T], res: Res[T]): Unit = {
    //    where += (args.length match {
    //      case 1 => (field, "= " + one2sql(args.head))
    //      case 0 => (field, "IS NULL ")
    //      case _ => (field, args.map(one2sql).mkString("IN (", ", ", ")"))
    //    })

    //    filters += Filters.eq[Int](field, 42)
    filters.add(args.length match {
      case 1 => res.eq(field, args.head)
      case 0 => res.eq(field, args.head)
      //      case 0 => (field, "IS NULL ")
      //      case _ => (field, args.map(one2sql).mkString("IN (", ", ", ")"))
    })
  }
