...
import molecule.document.mongodb.facade.{MongoDBConn_JVM, MongoHandler_JVM}
...
trait SqlBase_JVM extends SqlDataType_JVM with ModelUtils with BaseHelpers {
  ...
  private val connectionPool = mutable.HashMap.empty[UUID, Future[MongoDBConn_JVM]]
  ...
  protected def getConn(proxy: ConnProxy): Future[MongoDBConn_JVM] = {
  ...
  private def getFreshConn(proxy: ConnProxy): Future[MongoDBConn_JVM] = {
