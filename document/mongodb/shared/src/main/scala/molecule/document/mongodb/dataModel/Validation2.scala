...
object Validation2 extends DataModel(5) {
  ...
  trait MandatoryRefAB {
    val i    = oneInt
    val refA = one[RefA].owner .mandatory
  }
  trait MandatoryRefB {
    val i    = oneInt
    val refB = one[RefB].owner.mandatory
  }

  trait MandatoryRefsAB {
    val i     = oneInt
    val refsA = many[RefA].owner.mandatory
  }
  trait MandatoryRefsB {
    val i     = oneInt
    val refsB = many[RefB].owner.mandatory
  }
  ...
  trait Require {
    ...
    val refB = one[RefB].owner

    // Ref-only tuples
    val ref1 = one[RefB].owner .require(ref2)
    val ref2 = one[Enum].owner
  }
}
