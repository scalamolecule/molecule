...
case class Ref(
  ...
  override def toString: String = {
    ...
    s"""Ref("$ent", "$refAttr", "$ref", $relationship, $coords, ${oStr(reverseRef)}, $owner)"""
  }
...
case class SubQuery(
  ...
  isJoin: Boolean  // true for .join(), false for .select()
) extends Element {
  override def render(i: Int): String = {
    val indent = "  " * i
    s"""|${indent}SubQuery(
        |${indent}  List(
        |${renders(elements, i + 2)}), ${opt(optLimit)}, ${opt(optOffset)}, $isJoin)""".stripMargin
  }
  override def toString: String = render(0)
}
