...
////    case Generic(_, "e" | "e_", _, _, _, Eq1(List(_)), _) => err("unexpectedAppliedId",
//    case Generic(_, "e" | "e_", _, _, _, Eq, _) => err("unexpectedAppliedId",
//      s"""Applying an eid is only allowed for updates.""")
//    case ok                                               => ok
//  }
//
//  private def missingAppliedId: Boolean = model.elements.head match {
////    case Generic(_, "e" | "e_", _, _, _, Eq1(List(eid)), _) => true
//    case Generic(_, "e" | "e_", _, _, _, Eq, _) => true
//    case Generic(_, "e" | "e_", _, _, _, Eq_, _)      => true
...
//      case AttrValue | EntValue | EnumVal | IndexVal | Eq1(Seq(Qm)) | Distinct | NoValue => err(
//        "onlyAtomsWithValue",
//        "Update molecule can only have attributes with some value(s) applied/added/replaced etc."
//      )
//      case bi if bi.isInstanceOf[Bidirectional]                                          => err(
//        "onlyAtomsWithValue",
//        "Update molecule can only have attributes with some value(s) applied/added/replaced etc."
//      )
//      case other                                                                       => true
...
//    case Generic(_, attr, _, _, _, Eq1(vValues(eid :: _)), _) if datomGenerics.contains(attr) => err("noGenerics",
...
//      case Atom(nsFull, attr, mode, _, _, _, _, _) if mode != "tac" =>
...
//        case g@Generic(_, "e" | "e_", _, _, _, EntValue, _) => attrs :+ g
...
//      case Atom(nsFull, attr, _, tpe, 1, _, Eq1(vValues(vs)), _) if vs.length > 1 =>
...
////          case Atom(_, _, _, _, _, _, _, _, Seq(BiTargetRefAttr(_, attr)), _, _) => true
...
//////        // Base.attr.edge ...
//////        case Atom(_, _, _, _, _, _, _, _) if extractNs(edgeRefAttr) == edgeNs => true
//////      }
//////    }
////
...
////        // ... targetAttr
////        case Atom(edgeNs1, _, _, _, _, _, _, _, Seq(BiTargetRefAttr(_, attr)), _, _) if edgeNs1 == edgeNs => true
////      }
////
...
////
////        // Edge.prop ..?
////        case Atom(edgeNs, prop, _, _, _, _, _, _, Seq(BiEdgePropAttr(_)), _, _) =>
...
//////      case Generic(_, "e_", _, "e", _, Eq1(List(eid)), _) => // BiEdge
////      case Generic(_, "e_", _, "e", _, Eq, _) => // BiEdge
...
////        case Bond(edgeNs1, _, _, _, Seq(BiTargetRef(_, _))) if edgeNs1 == edgeNs => true
////        // ... targetAttr
////        case Atom(edgeNs1, _, _, _, _, _, _, _, Seq(BiTargetRefAttr(_, _)), _, _) if edgeNs1 == edgeNs => true
////      }
////
////      elements.collectFirst {
////        // Base.attr.Edge ..?
