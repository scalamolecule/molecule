      ...
      case a: AttrOne => a match {
        case a: AttrOneMan =>
          ...
          a match {
            ...
            case a: AttrOneManDuration       => a.copy(ns = ns, attr = attr1, filterAttr = optFilterAttr)
            case a: AttrOneManInstant        => a.copy(ns = ns, attr = attr1, filterAttr = optFilterAttr)
            case a: AttrOneManLocalDate      => a.copy(ns = ns, attr = attr1, filterAttr = optFilterAttr)
            case a: AttrOneManLocalTime      => a.copy(ns = ns, attr = attr1, filterAttr = optFilterAttr)
            case a: AttrOneManLocalDateTime  => a.copy(ns = ns, attr = attr1, filterAttr = optFilterAttr)
            case a: AttrOneManOffsetTime     => a.copy(ns = ns, attr = attr1, filterAttr = optFilterAttr)
            case a: AttrOneManOffsetDateTime => a.copy(ns = ns, attr = attr1, filterAttr = optFilterAttr)
            case a: AttrOneManZonedDateTime  => a.copy(ns = ns, attr = attr1, filterAttr = optFilterAttr)
        ...
        case a: AttrOneOpt =>
          ...
          a match {
            ...
            case a: AttrOneOptDuration       => a.copy(ns = ns, attr = attr)
            case a: AttrOneOptInstant        => a.copy(ns = ns, attr = attr)
            case a: AttrOneOptLocalDate      => a.copy(ns = ns, attr = attr)
            case a: AttrOneOptLocalTime      => a.copy(ns = ns, attr = attr)
            case a: AttrOneOptLocalDateTime  => a.copy(ns = ns, attr = attr)
            case a: AttrOneOptOffsetTime     => a.copy(ns = ns, attr = attr)
            case a: AttrOneOptOffsetDateTime => a.copy(ns = ns, attr = attr)
            case a: AttrOneOptZonedDateTime  => a.copy(ns = ns, attr = attr)
        ...
        case a: AttrOneTac =>
          ...
          a match {
            ...
            case a: AttrOneTacDuration       => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrOneTacInstant        => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrOneTacLocalDate      => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrOneTacLocalTime      => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrOneTacLocalDateTime  => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrOneTacOffsetTime     => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrOneTacOffsetDateTime => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrOneTacZonedDateTime  => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
      ...
      case a: AttrSet => a match {
        case a: AttrSetMan =>
          ...
          a match {
            ...
            case a: AttrSetManDuration       => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetManInstant        => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetManLocalDate      => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetManLocalTime      => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetManLocalDateTime  => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetManOffsetTime     => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetManOffsetDateTime => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetManZonedDateTime  => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
        ...
        case a: AttrSetOpt =>
          ...
          a match {
            ...
            case a: AttrSetOptDuration       => a.copy(ns = ns, attr = attr)
            case a: AttrSetOptInstant        => a.copy(ns = ns, attr = attr)
            case a: AttrSetOptLocalDate      => a.copy(ns = ns, attr = attr)
            case a: AttrSetOptLocalTime      => a.copy(ns = ns, attr = attr)
            case a: AttrSetOptLocalDateTime  => a.copy(ns = ns, attr = attr)
            case a: AttrSetOptOffsetTime     => a.copy(ns = ns, attr = attr)
            case a: AttrSetOptOffsetDateTime => a.copy(ns = ns, attr = attr)
            case a: AttrSetOptZonedDateTime  => a.copy(ns = ns, attr = attr)
        ...
        case a: AttrSetTac =>
          ...
          a match {
            ...
            case a: AttrSetTacDuration       => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetTacInstant        => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetTacLocalDate      => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetTacLocalTime      => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetTacLocalDateTime  => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetTacOffsetTime     => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetTacOffsetDateTime => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
            case a: AttrSetTacZonedDateTime  => a.copy(ns = ns, attr = attr, filterAttr = optFilterAttr)
