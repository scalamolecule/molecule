...
class ResolveUpdate(
  ...
  private def resolveAttrSetRemove(a: AttrSetMan): Unit = {
    val (ns, attr) = (a.ns, a.attr)
    a match {
      case a: AttrSetManID             => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformID, extsID, one2jsonID)
      case a: AttrSetManString         => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformString, extsString, one2jsonString)
      case a: AttrSetManInt            => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformInt, extsInt, one2jsonInt)
      case a: AttrSetManLong           => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformLong, extsLong, one2jsonLong)
      case a: AttrSetManFloat          => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformFloat, extsFloat, one2jsonFloat)
      case a: AttrSetManDouble         => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformDouble, extsDouble, one2jsonDouble)
      case a: AttrSetManBoolean        => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformBoolean, extsBoolean, one2jsonBoolean)
      case a: AttrSetManBigInt         => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformBigInt, extsBigInt, one2jsonBigInt)
      case a: AttrSetManBigDecimal     => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformBigDecimal, extsBigDecimal, one2jsonBigDecimal)
      case a: AttrSetManDate           => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformDate, extsDate, one2jsonDate)
      case a: AttrSetManDuration       => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformDuration, extsDuration, one2jsonDuration)
      case a: AttrSetManInstant        => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformInstant, extsInstant, one2jsonInstant)
      case a: AttrSetManLocalDate      => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformLocalDate, extsLocalDate, one2jsonLocalDate)
      case a: AttrSetManLocalTime      => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformLocalTime, extsLocalTime, one2jsonLocalTime)
      case a: AttrSetManLocalDateTime  => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformLocalDateTime, extsLocalDateTime, one2jsonLocalDateTime)
      case a: AttrSetManOffsetTime     => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformOffsetTime, extsOffsetTime, one2jsonOffsetTime)
      case a: AttrSetManOffsetDateTime => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformOffsetDateTime, extsOffsetDateTime, one2jsonOffsetDateTime)
      case a: AttrSetManZonedDateTime  => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformZonedDateTime, extsZonedDateTime, one2jsonZonedDateTime)
      case a: AttrSetManUUID           => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformUUID, extsUUID, one2jsonUUID)
      case a: AttrSetManURI            => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformURI, extsURI, one2jsonURI)
      case a: AttrSetManByte           => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformByte, extsByte, one2jsonByte)
      case a: AttrSetManShort          => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformShort, extsShort, one2jsonShort)
      case a: AttrSetManChar           => updateSetRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformChar, extsChar, one2jsonChar)
    }
  }
  ...
  private def resolveAttrSeqRemove(a: AttrSeqMan): Unit = {
    val (ns, attr) = (a.ns, a.attr)
    a match {
      case a: AttrSeqManID             => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformID, extsID, one2jsonID)
      case a: AttrSeqManString         => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformString, extsString, one2jsonString)
      case a: AttrSeqManInt            => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformInt, extsInt, one2jsonInt)
      case a: AttrSeqManLong           => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformLong, extsLong, one2jsonLong)
      case a: AttrSeqManFloat          => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformFloat, extsFloat, one2jsonFloat)
      case a: AttrSeqManDouble         => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformDouble, extsDouble, one2jsonDouble)
      case a: AttrSeqManBoolean        => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformBoolean, extsBoolean, one2jsonBoolean)
      case a: AttrSeqManBigInt         => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformBigInt, extsBigInt, one2jsonBigInt)
      case a: AttrSeqManBigDecimal     => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformBigDecimal, extsBigDecimal, one2jsonBigDecimal)
      case a: AttrSeqManDate           => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformDate, extsDate, one2jsonDate)
      case a: AttrSeqManDuration       => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformDuration, extsDuration, one2jsonDuration)
      case a: AttrSeqManInstant        => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformInstant, extsInstant, one2jsonInstant)
      case a: AttrSeqManLocalDate      => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformLocalDate, extsLocalDate, one2jsonLocalDate)
      case a: AttrSeqManLocalTime      => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformLocalTime, extsLocalTime, one2jsonLocalTime)
      case a: AttrSeqManLocalDateTime  => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformLocalDateTime, extsLocalDateTime, one2jsonLocalDateTime)
      case a: AttrSeqManOffsetTime     => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformOffsetTime, extsOffsetTime, one2jsonOffsetTime)
      case a: AttrSeqManOffsetDateTime => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformOffsetDateTime, extsOffsetDateTime, one2jsonOffsetDateTime)
      case a: AttrSeqManZonedDateTime  => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformZonedDateTime, extsZonedDateTime, one2jsonZonedDateTime)
      case a: AttrSeqManUUID           => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformUUID, extsUUID, one2jsonUUID)
      case a: AttrSeqManURI            => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformURI, extsURI, one2jsonURI)
      case a: AttrSeqManByte           => throw ModelError(s"Operations on byte arrays (${a.ns}.${a.attr}) not allowed.")
      case a: AttrSeqManShort          => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformShort, extsShort, one2jsonShort)
      case a: AttrSeqManChar           => updateSeqRemove(ns, attr, a.vs.head, a.refNs, a.owner, transformChar, extsChar, one2jsonChar)
    }
  }
