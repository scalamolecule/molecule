...
class UpdateExtraction(
  ...
  final def resolve(
    ...
    elements match {
      case element :: tail => element match {
        ...
        case TxMetaData(es) =>
          if (data.isEmpty)
            throw ModelError(s"Can't $update tx meta data only.")
          val (eids1, filters1, data1) = extractSubElements(es)
  ...
  private def oneMan(
    ...
    if (isUpsert) {
      ...
      val dummyFilterAttr = AttrOneTacInt(dataAttr.ns, dataAttr.attr, V, Nil, None, Nil, None, None)
  ...
  private def oneApply(attr: AttrOneMan): (String, String, String, Seq[AnyRef], Boolean) = attr match {
    ...
    case AttrOneManInt(_, _, _, vs, _, _, _, _)        => addOneV[Int](attr, vs, valueInt)
    case AttrOneManLong(_, _, _, vs, _, _, _, _)       => addOneV[Long](attr, vs, valueLong)
    case AttrOneManFloat(_, _, _, vs, _, _, _, _)      => addOneV[Float](attr, vs, valueFloat)
    case AttrOneManDouble(_, _, _, vs, _, _, _, _)     => addOneV[Double](attr, vs, valueDouble)
    case AttrOneManBoolean(_, _, _, vs, _, _, _, _)    => addOneV[Boolean](attr, vs, valueBoolean)
    case AttrOneManBigInt(_, _, _, vs, _, _, _, _)     => addOneV[BigInt](attr, vs, valueBigInt)
    case AttrOneManBigDecimal(_, _, _, vs, _, _, _, _) => addOneV[BigDecimal](attr, vs, valueBigDecimal)
    case AttrOneManDate(_, _, _, vs, _, _, _, _)       => addOneV[Date](attr, vs, valueDate)
    case AttrOneManUUID(_, _, _, vs, _, _, _, _)       => addOneV[UUID](attr, vs, valueUUID)
    case AttrOneManURI(_, _, _, vs, _, _, _, _)        => addOneV[URI](attr, vs, valueURI)
    case AttrOneManByte(_, _, _, vs, _, _, _, _)       => addOneV[Byte](attr, vs, valueByte)
    case AttrOneManShort(_, _, _, vs, _, _, _, _)      => addOneV[Short](attr, vs, valueShort)
    case AttrOneManChar(_, _, _, vs, _, _, _, _)       => addOneV[Char](attr, vs, valueChar)
  }
  ...
  private def oneTac(
    ...
    filterAttr match {
      ...
      case AttrOneTacLong("_Generic", "e", Appl, _, _, _, _, _) => throw ModelError(
  ...
  private def setAdd(
    ...
    case AttrSetManBigInt(_, _, _, sets, _, _, _, _)     => addSetVs[BigInt](attr, sets, valueBigInt, retractCur)
    case AttrSetManBigDecimal(_, _, _, sets, _, _, _, _) => addSetVs[BigDecimal](attr, sets, valueBigDecimal, retractCur)
    case AttrSetManDate(_, _, _, sets, _, _, _, _)       => addSetVs[Date](attr, sets, valueDate, retractCur)
    case AttrSetManUUID(_, _, _, sets, _, _, _, _)       => addSetVs[UUID](attr, sets, valueUUID, retractCur)
    case AttrSetManURI(_, _, _, sets, _, _, _, _)        => addSetVs[URI](attr, sets, valueURI, retractCur)
    case AttrSetManByte(_, _, _, sets, _, _, _, _)       => addSetVs[Byte](attr, sets, valueByte, retractCur)
    case AttrSetManShort(_, _, _, sets, _, _, _, _)      => addSetVs[Short](attr, sets, valueShort, retractCur)
    case AttrSetManChar(_, _, _, sets, _, _, _, _)       => addSetVs[Char](attr, sets, valueChar, retractCur)
  }
  ...
  private def setSwap(attr: AttrSetMan): Seq[(String, String, String, Seq[AnyRef], Boolean)] = attr match {
    ...
    case AttrSetManBigInt(_, _, _, sets, _, _, _, _)     => swapSetVs[BigInt](attr, sets, valueBigInt)
    case AttrSetManBigDecimal(_, _, _, sets, _, _, _, _) => swapSetVs[BigDecimal](attr, sets, valueBigDecimal)
    case AttrSetManDate(_, _, _, sets, _, _, _, _)       => swapSetVs[Date](attr, sets, valueDate)
    case AttrSetManUUID(_, _, _, sets, _, _, _, _)       => swapSetVs[UUID](attr, sets, valueUUID)
    case AttrSetManURI(_, _, _, sets, _, _, _, _)        => swapSetVs[URI](attr, sets, valueURI)
    case AttrSetManByte(_, _, _, sets, _, _, _, _)       => swapSetVs[Byte](attr, sets, valueByte)
    case AttrSetManShort(_, _, _, sets, _, _, _, _)      => swapSetVs[Short](attr, sets, valueShort)
    case AttrSetManChar(_, _, _, sets, _, _, _, _)       => swapSetVs[Char](attr, sets, valueChar)
  }
  ...
  private def setRemove(attr: AttrSetMan): Seq[(String, String, String, Seq[AnyRef], Boolean)] = attr match {
    ...
    case AttrSetManBigInt(_, _, _, Seq(set), _, _, _, _)     => removeSetVs[BigInt](attr, set, valueBigInt)
    case AttrSetManBigDecimal(_, _, _, Seq(set), _, _, _, _) => removeSetVs[BigDecimal](attr, set, valueBigDecimal)
    case AttrSetManDate(_, _, _, Seq(set), _, _, _, _)       => removeSetVs[Date](attr, set, valueDate)
    case AttrSetManUUID(_, _, _, Seq(set), _, _, _, _)       => removeSetVs[UUID](attr, set, valueUUID)
    case AttrSetManURI(_, _, _, Seq(set), _, _, _, _)        => removeSetVs[URI](attr, set, valueURI)
