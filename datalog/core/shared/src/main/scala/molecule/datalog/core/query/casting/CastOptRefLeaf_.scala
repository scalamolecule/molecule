...
trait CastOptRefLeaf_
  ...
  final private def resolve(
    ...
    val handleMap = handleRows(cast)
    //    val handleMap = optComparator.fold(
    //      handleRows(arity, cast)
    //    )(comparator =>
    //      handleRowsSorted(arity, cast, comparator)
    //    )
    resolveOptional(handleMap)
  }

  private def handleRowsOLD(
    arity: Int,
    cast: java.util.Iterator[_] => Any
  ): jList[_] => Option[Any] = {
  ...
  private def handleRows(
    ...
    (optionalData: jMap[_, _]) =>
      println("optionalData: " + optionalData)
      Some(cast(optionalData.values().iterator()))
  }

  final private def resolveOptional(
    handleMap: jMap[_, _] => Option[Any]
  ): jIterator[_] => Option[Any] = {
    (it: jIterator[_]) =>
      try {
        it.next match {
          case map: jMap[_, _] =>
            println(s"D2  $map  " + map.getClass.getSimpleName)
            handleMap(map)

          case x =>
            println(s"D3  $x  " + x.getClass.getSimpleName)
  ...
  //            }
  //            if (coalescedSet.isEmpty) None else Some(coalescedSet)
  //
  //          } else {
  //            rows.asScala.toList.foreach {
  //              case row: jMap[_, _] =>
