...
object FilterOne_Date_ extends DatomicTestSuite {
  ...
  override lazy val tests = Tests {
    ...
    "Optional" - types { implicit conn =>
      ...
      for {
        ...
        // Distinct result even with redundant None's (two i = 4 with no date value)
        _ <- Ns.i.insert(4).transact
        _ <- Ns.i.>=(3).a1.date_?.query.get.map(_ ==> List(
          (3, Some(date3)),
          (4, None),
        ))
      } yield ()
