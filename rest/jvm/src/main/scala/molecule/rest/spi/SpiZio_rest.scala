...
//    sync2zio[List[Tpl]]((conn: DatomicConn_JVM) => SpiSync_rest.query_get(q)(conn))
    ???
  }
...
//    sync2zio[Unit]((conn: DatomicConn_JVM) => SpiSync_rest.query_subscribe(q, callback)(conn))
    ???
  }
  override def query_unsubscribe[Tpl](
    q: Query[Tpl]
  ): ZIO[Conn, MoleculeError, Unit] = {
//    sync2zio[Unit]((conn: DatomicConn_JVM) => SpiSync_rest.query_unsubscribe(q)(conn))
    ???
  }
...
//    sync2zio[Unit]((conn: DatomicConn_JVM) => SpiSync_rest.query_inspect(q)(conn))
    ???
  }
...
//    sync2zio[(List[Tpl], Int, Boolean)]((conn: DatomicConn_JVM) => SpiSync_rest.queryOffset_get(q)(conn))
    ???
  }
...
//    sync2zio[(List[Tpl], String, Boolean)]((conn: DatomicConn_JVM) => SpiSync_rest.queryCursor_get(q)(conn))
    ???
  }
...
//    sync2zio[Unit]((conn: DatomicConn_JVM) => SpiSync_rest.save_inspect(save)(conn))
    ???
  }

  override def save_validate(save: Save): ZIO[Conn, MoleculeError, Map[String, Seq[String]]] = {
//    sync2zio[Map[String, Seq[String]]]((conn: DatomicConn_JVM) => SpiSync_rest.save_validate(save)(conn))
    ???
  }
...
//    sync2zio[Unit]((conn: DatomicConn_JVM) => SpiSync_rest.insert_inspect(insert)(conn))
    ???
  }

  override def insert_validate(insert: Insert): ZIO[Conn, MoleculeError, Seq[(Int, Seq[InsertError])]] = {
//    sync2zio[Seq[(Int, Seq[InsertError])]]((conn: DatomicConn_JVM) => SpiSync_rest.insert_validate(insert)(conn))
    ???
  }
...
//    sync2zio[Unit]((conn: DatomicConn_JVM) => SpiSync_rest.update_inspect(update)(conn))
    ???
  }

  override def update_validate(update: Update): ZIO[Conn, MoleculeError, Map[String, Seq[String]]] = {
//    sync2zio[Map[String, Seq[String]]]((conn: DatomicConn_JVM) => SpiSync_rest.update_validate(update)(conn))
    ???
  }
...
//    sync2zio[Unit]((conn: DatomicConn_JVM) => SpiSync_rest.delete_inspect(delete)(conn))
    ???
  }
...
//        SpiSync_rest.fallback_rawQuery(query, debug)(conn)
//      ))
//    } yield result
    ???
  }
