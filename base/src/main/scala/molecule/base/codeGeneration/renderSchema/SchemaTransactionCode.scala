...
import molecule.base.codeGeneration.parseDataModel.parserAST.{BackRef, DefAttr, Enum, Namespace, Optional, ParseModel}
import molecule.base.codeGeneration.parseDataModel.parserAST._


case class SchemaTransactionCode(d: ParseModel) extends MetaSchemaData {
    ...
    d.nss.collectFirst {
      case ns if ns.attrs.collectFirst {
        ...
        val moleculeMetaNs = Namespace("molecule", None, "molecule_Meta", None, None, Seq(
          Ref("otherEdge", 1, "OneRefAttr", "OneRef", "Long", "", "molecule_Meta", Seq(
  ...
  def attrStmts(ns: String, a: DefAttr, isClient: Boolean): String = {
    ...
    val stmts = a match {
      case Val(_, _, clazz, _, _, t, options, _, _, _, _) if clazz.take(3) == "One" => Seq(tpe(t), card("one")) ++ opts(options)
      case Val(_, _, _, _, _, t, options, _, _, _, _)                               => Seq(tpe(t), card("many")) ++ opts(options)
  ...
  def attributeDefinition0(isClient: Boolean): String = nss
    ...
    .map { ns =>
      ...
      val attrs  = ns.attrs.flatMap {
        case Val(_, _, _, _, _, "bytes", _, _, _, _, _) if isClient => Nil
        case _: BackRef                                          => Nil
        case e: Enum                                             =>
