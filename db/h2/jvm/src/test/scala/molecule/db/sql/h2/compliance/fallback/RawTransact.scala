...
class RawTransact extends MUnit with DbProviders_h2 with TestUtils {
  ...
  "Insert" - types { implicit conn =>
    for {
      ...
      _ <- rawTransact(s"insert into Entity (int           ) values ($int1)")
      _ <- rawTransact(s"insert into Entity (long          ) values ($long1)")
      _ <- rawTransact(s"insert into Entity (float         ) values ($float1)")
      _ <- rawTransact(s"insert into Entity (double        ) values ($double1)")
      _ <- rawTransact(s"insert into Entity (boolean       ) values ($boolean1)")
      _ <- rawTransact(s"insert into Entity (bigInt        ) values ($bigInt1)")
      _ <- rawTransact(s"insert into Entity (bigDecimal    ) values ($bigDecimal1)")
      _ <- rawTransact(s"insert into Entity (date          ) values (${date1.getTime})") // epoch time saved
      ...
      _ <- rawTransact(s"insert into Entity (char          ) values ('$char1')")
  ...
  "Insert Set" - types { implicit conn =>
    for {
      _ <- rawTransact(s"insert into Entity (stringSet        ) values (array['$string1', '$string2'])")
      _ <- rawTransact(s"insert into Entity (intSet           ) values (array[$int1, $int2])")
      _ <- rawTransact(s"insert into Entity (longSet          ) values (array[$long1, $long2])")
      _ <- rawTransact(s"insert into Entity (floatSet         ) values (array[$float1, $float2])")
      _ <- rawTransact(s"insert into Entity (doubleSet        ) values (array[$double1, $double2])")
      _ <- rawTransact(s"insert into Entity (booleanSet       ) values (array[$boolean1, $boolean2])")
      _ <- rawTransact(s"insert into Entity (bigIntSet        ) values (array[$bigInt1, $bigInt2])")
      _ <- rawTransact(s"insert into Entity (bigDecimalSet    ) values (array[$bigDecimal1, $bigDecimal2])")
      _ <- rawTransact(s"insert into Entity (dateSet          ) values (array[${date1.getTime}, ${date2.getTime}])")
      _ <- rawTransact(s"insert into Entity (durationSet      ) values (array['${duration1.toString}', '${duration2.toString}'])")
      _ <- rawTransact(s"insert into Entity (instantSet       ) values (array['${instant1.toString}', '${instant2.toString}'])")
      _ <- rawTransact(s"insert into Entity (localDateSet     ) values (array['${localDate1.toString}', '${localDate2.toString}'])")
      _ <- rawTransact(s"insert into Entity (localTimeSet     ) values (array['${localTime1.toString}', '${localTime2.toString}'])")
      _ <- rawTransact(s"insert into Entity (localDateTimeSet ) values (array['${localDateTime1.toString}', '${localDateTime2.toString}'])")
      _ <- rawTransact(s"insert into Entity (offsetTimeSet    ) values (array['${offsetTime1.toString}', '${offsetTime2.toString}'])")
      _ <- rawTransact(s"insert into Entity (offsetDateTimeSet) values (array['${offsetDateTime1.toString}', '${offsetDateTime2.toString}'])")
      _ <- rawTransact(s"insert into Entity (zonedDateTimeSet ) values (array['${zonedDateTime1.toString}', '${zonedDateTime2.toString}'])")
      _ <- rawTransact(s"insert into Entity (uuidSet          ) values (array['$uuid1', '$uuid2'])")
      _ <- rawTransact(s"insert into Entity (uriSet           ) values (array['$uri1', '$uri2'])")
      _ <- rawTransact(s"insert into Entity (byteSet          ) values (array[$byte1, $byte2])")
      _ <- rawTransact(s"insert into Entity (shortSet         ) values (array[$short1, $short2])")
      _ <- rawTransact(s"insert into Entity (charSet          ) values (array['$char1', '$char2'])")
  ...
  "Update" - types { implicit conn =>
    for {
      ...
      _ <- rawTransact(s"update Entity set int            = $int2                         where id = $id2")
      _ <- rawTransact(s"update Entity set long           = $long2                        where id = $id3")
      _ <- rawTransact(s"update Entity set float          = $float2                       where id = $id4")
      _ <- rawTransact(s"update Entity set double         = $double2                      where id = $id5")
      _ <- rawTransact(s"update Entity set boolean        = $boolean2                     where id = $id6")
      _ <- rawTransact(s"update Entity set bigInt         = $bigInt2                      where id = $id7")
      _ <- rawTransact(s"update Entity set bigDecimal     = $bigDecimal2                  where id = $id8")
      _ <- rawTransact(s"update Entity set date           = ${date2.getTime}              where id = $id9")
      ...
      _ <- rawTransact(s"update Entity set char           = '$char2'                      where id = $id22")
  ...
  "Delete" - types { implicit conn =>
    for {
      ...
      case List(id2, _) <- Entity.int.insert(int1, int2).transact.map(_.ids)
      case List(id3, _) <- Entity.long.insert(long1, long2).transact.map(_.ids)
      case List(id4, _) <- Entity.float.insert(float1, float2).transact.map(_.ids)
      case List(id5, _) <- Entity.double.insert(double1, double2).transact.map(_.ids)
      case List(id6, _) <- Entity.boolean.insert(boolean1, boolean2).transact.map(_.ids)
      case List(id7, _) <- Entity.bigInt.insert(bigInt1, bigInt2).transact.map(_.ids)
      case List(id8, _) <- Entity.bigDecimal.insert(bigDecimal1, bigDecimal2).transact.map(_.ids)
      case List(id9, _) <- Entity.date.insert(date1, date2).transact.map(_.ids)
      case List(id10, _) <- Entity.duration.insert(duration1, duration2).transact.map(_.ids)
      case List(id11, _) <- Entity.instant.insert(instant1, instant2).transact.map(_.ids)
      case List(id12, _) <- Entity.localDate.insert(localDate1, localDate2).transact.map(_.ids)
      case List(id13, _) <- Entity.localTime.insert(localTime1, localTime2).transact.map(_.ids)
      case List(id14, _) <- Entity.localDateTime.insert(localDateTime1, localDateTime2).transact.map(_.ids)
      case List(id15, _) <- Entity.offsetTime.insert(offsetTime1, offsetTime2).transact.map(_.ids)
      case List(id16, _) <- Entity.offsetDateTime.insert(offsetDateTime1, offsetDateTime2).transact.map(_.ids)
      case List(id17, _) <- Entity.zonedDateTime.insert(zonedDateTime1, zonedDateTime2).transact.map(_.ids)
      case List(id18, _) <- Entity.uuid.insert(uuid1, uuid2).transact.map(_.ids)
      case List(id19, _) <- Entity.uri.insert(uri1, uri2).transact.map(_.ids)
      case List(id20, _) <- Entity.byte.insert(byte1, byte2).transact.map(_.ids)
      case List(id21, _) <- Entity.short.insert(short1, short2).transact.map(_.ids)
      case List(id22, _) <- Entity.char.insert(char1, char2).transact.map(_.ids)
