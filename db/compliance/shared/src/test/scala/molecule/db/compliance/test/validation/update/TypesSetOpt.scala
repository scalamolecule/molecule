...
case class TypesSetOpt(
  ...
  "Types" - validation {
    for {
      id <- Tpe.stringSet_?(Some(Set("d"))).save.transact.map(_.id)

      _ <- Tpe(id).stringSet_?(Some(Set("a", "b", "d"))).update.transact
      ...
      id <- Tpe.intSet_?(Some(Set(4))).save.transact.map(_.id)
      _ <- Tpe(id).intSet_?(Some(Set(1, 2, 4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.longSet_?(Some(Set(4L))).save.transact.map(_.id)
      _ <- Tpe(id).longSet_?(Some(Set(1L, 2L, 4L))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.floatSet_?(Some(Set(float4))).save.transact.map(_.id)
      _ <- Tpe(id).floatSet_?(Some(Set(float1, float2, float4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      _ <- Tpe(id).doubleSet_?(Some(Set(double1, double2, double4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      _ <- Tpe(id).booleanSet_?(Some(Set(true, false))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.bigIntSet_?(Some(Set(bigInt4))).save.transact.map(_.id)
      _ <- Tpe(id).bigIntSet_?(Some(Set(bigInt1, bigInt2, bigInt4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.bigDecimalSet_?(Some(Set(bigDecimal4))).save.transact.map(_.id)
      _ <- Tpe(id).bigDecimalSet_?(Some(Set(bigDecimal1, bigDecimal2, bigDecimal4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.dateSet_?(Some(Set(date4))).save.transact.map(_.id)
      _ <- Tpe(id).dateSet_?(Some(Set(date1, date2, date4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.durationSet_?(Some(Set(duration4))).save.transact.map(_.id)
      _ <- Tpe(id).durationSet_?(Some(Set(duration1, duration2, duration4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.instantSet_?(Some(Set(instant4))).save.transact.map(_.id)
      _ <- Tpe(id).instantSet_?(Some(Set(instant1, instant2, instant4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.localDateSet_?(Some(Set(localDate4))).save.transact.map(_.id)
      _ <- Tpe(id).localDateSet_?(Some(Set(localDate1, localDate2, localDate4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.localTimeSet_?(Some(Set(localTime4))).save.transact.map(_.id)
      _ <- Tpe(id).localTimeSet_?(Some(Set(localTime1, localTime2, localTime4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.localDateTimeSet_?(Some(Set(localDateTime4))).save.transact.map(_.id)
      _ <- Tpe(id).localDateTimeSet_?(Some(Set(localDateTime1, localDateTime2, localDateTime4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.offsetTimeSet_?(Some(Set(offsetTime4))).save.transact.map(_.id)
      _ <- Tpe(id).offsetTimeSet_?(Some(Set(offsetTime1, offsetTime2, offsetTime4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.offsetDateTimeSet_?(Some(Set(offsetDateTime4))).save.transact.map(_.id)
      _ <- Tpe(id).offsetDateTimeSet_?(Some(Set(offsetDateTime1, offsetDateTime2, offsetDateTime4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.zonedDateTimeSet_?(Some(Set(zonedDateTime4))).save.transact.map(_.id)
      _ <- Tpe(id).zonedDateTimeSet_?(Some(Set(zonedDateTime1, zonedDateTime2, zonedDateTime4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.uuidSet_?(Some(Set(UUID.fromString("aaaaaaaa-aaaa-aaaa-aaaa-dddddddddddd")))).save.transact.map(_.id)
      _ <- Tpe(id).uuidSet_?(Some(Set(UUID.fromString("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa")))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      uri2 = new URI("ab")
      uri4 = new URI("abcd")
      id <- Tpe.uriSet_?(Some(Set(uri4))).save.transact.map(_.id)
      _ <- Tpe(id).uriSet_?(Some(Set(uri1, uri2, uri4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.byteSet_?(Some(Set(byte4))).save.transact.map(_.id)
      _ <- Tpe(id).byteSet_?(Some(Set(byte1, byte2, byte4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.shortSet_?(Some(Set(short4))).save.transact.map(_.id)
      _ <- Tpe(id).shortSet_?(Some(Set(short1, short2, short4))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
      ...
      id <- Tpe.charSet_?(Some(Set('d'))).save.transact.map(_.id)
      _ <- Tpe(id).charSet_?(Some(Set('a', 'b', 'd'))).update.transact
        .map(_ ==> "Unexpected success").recover {
          case ValidationErrors(errorMap) =>
            errorMap.head._2 ==> Seq(
