...
import molecule.db.sql.core.query.{Model2SqlQuery, SqlQueryBase, SqlQueryResolveCursor, SqlQueryResolveOffset}
...
import molecule.db.sql.core.transaction.{CachedConnection, SqlUpdateSetValidator}
...
trait SpiBaseJVM_sync
  ...
  override def query_get[Tpl](q0: Query[Tpl])(implicit conn0: Conn): List[Tpl] = {
    ...
    if (q0.doInspect)
      query_inspect(q0)
    val queryClean = q0.copy(elements = keywordsSuffixed(q0.elements, conn.proxy))
  ...
  override def queryOffset_get[Tpl](query: QueryOffset[Tpl])
    ...
    val queryClean = query.copy(elements = keywordsSuffixed(query.elements, conn.proxy))
    val m2q        = getModel2SqlQuery(queryClean.elements)
  ...
  override def save_transact(save: Save)(implicit conn0: Conn): TxReport = {
    ...
    if (save.doInspect)
      save_inspect(save)
    val saveClean = save.copy(elements = keywordsSuffixed(save.elements, conn.proxy))
  ...
  override def insert_transact(insert: Insert)(implicit conn0: Conn): TxReport = {
    ...
    val insertClean = insert.copy(elements = keywordsSuffixed(insert.elements, conn.proxy))
  ...
  override def update_transact(update: Update)(implicit conn0: Conn): TxReport = {
    ...
    val updateClean = update.copy(elements = keywordsSuffixed(update.elements, conn.proxy))
  ...
  override def delete_transact(delete: Delete)(implicit conn0: Conn): TxReport = {
    ...
    val deleteClean = delete.copy(elements = keywordsSuffixed(delete.elements, conn.proxy))
