...
trait Spi_postgres_zio extends Spi_zio with SpiBase_postgres_zio with StreamingJdbc with ModelUtils {
  ...
  override def query_inspect[Tpl](q: Query[Tpl]): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_postgres_sync.query_inspect(q)(conn))
  }
  ...
  override def queryOffset_inspect[Tpl](q: QueryOffset[Tpl]): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_postgres_sync.queryOffset_inspect(q)(conn))
  }
  ...
  override def queryCursor_inspect[Tpl](q: QueryCursor[Tpl]): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_postgres_sync.queryCursor_inspect(q)(conn))
  }
  ...
  override def save_inspect(save: Save): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_postgres_sync.save_inspect(save)(conn))
  }
  ...
  override def insert_inspect(insert: Insert): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_postgres_sync.insert_inspect(insert)(conn))
  }
  ...
  override def update_inspect(update: Update): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_postgres_sync.update_inspect(update)(conn))
  }
  ...
  override def delete_inspect(delete: Delete): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_postgres_sync.delete_inspect(delete)(conn))
  }
