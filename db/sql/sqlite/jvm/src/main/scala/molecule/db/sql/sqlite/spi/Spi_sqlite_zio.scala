...
trait Spi_sqlite_zio extends Spi_zio with SpiBase_sqlite_zio with StreamingJdbc with ModelUtils {
  ...
  override def query_get[Tpl](q: Query[Tpl]): ZIO[Conn, MoleculeError, List[Tpl]] = {
    sync2zio[List[Tpl]]((conn: JdbcConn_JVM) => Spi_sqlite_sync.query_get(q)(conn))
  }

  override def query_inspect[Tpl](q: Query[Tpl]): ZIO[Conn, MoleculeError, String] = {
    sync2zio[String]((conn: JdbcConn_JVM) => Spi_sqlite_sync.query_inspect(q)(conn))
  }


  override def queryOffset_get[Tpl](q: QueryOffset[Tpl]): ZIO[Conn, MoleculeError, (List[Tpl], Int, Boolean)] = {
    sync2zio[(List[Tpl], Int, Boolean)]((conn: JdbcConn_JVM) => Spi_sqlite_sync.queryOffset_get(q)(conn))
  }

  override def queryOffset_inspect[Tpl](q: QueryOffset[Tpl]): ZIO[Conn, MoleculeError, String] = {
    sync2zio[String]((conn: JdbcConn_JVM) => Spi_sqlite_sync.queryOffset_inspect(q)(conn))
  }


  override def queryCursor_get[Tpl](q: QueryCursor[Tpl]): ZIO[Conn, MoleculeError, (List[Tpl], String, Boolean)] = {
    sync2zio[(List[Tpl], String, Boolean)]((conn: JdbcConn_JVM) => Spi_sqlite_sync.queryCursor_get(q)(conn))
  }

  override def queryCursor_inspect[Tpl](q: QueryCursor[Tpl]): ZIO[Conn, MoleculeError, String] = {
    sync2zio[String]((conn: JdbcConn_JVM) => Spi_sqlite_sync.queryCursor_inspect(q)(conn))
  }
  ...
  override def query_subscribe[Tpl](q: Query[Tpl], callback: List[Tpl] => Unit): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_sqlite_sync.query_subscribe(q, callback)(conn))
  }

  override def query_unsubscribe[Tpl](q: Query[Tpl]): ZIO[Conn, MoleculeError, Unit] = {
    sync2zio[Unit]((conn: JdbcConn_JVM) => Spi_sqlite_sync.query_unsubscribe(q)(conn))
  }
  ...
  override def save_transact(save: Save): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => Spi_sqlite_sync.save_transact(save)(conn)
  ...
  override def save_inspect(save: Save): ZIO[Conn, MoleculeError, String] = {
    sync2zio[String]((conn: JdbcConn_JVM) => Spi_sqlite_sync.save_inspect(save)(conn))
  }

  override def save_validate(save: Save): ZIO[Conn, MoleculeError, Map[String, Seq[String]]] = {
    sync2zio[Map[String, Seq[String]]]((conn: JdbcConn_JVM) => Spi_sqlite_sync.save_validate(save)(conn))
  }
  ...
  override def insert_transact(insert: Insert): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => Spi_sqlite_sync.insert_transact(insert)(conn)
  ...
  override def insert_inspect(insert: Insert): ZIO[Conn, MoleculeError, String] = {
    sync2zio[String]((conn: JdbcConn_JVM) => Spi_sqlite_sync.insert_inspect(insert)(conn))
  }

  override def insert_validate(insert: Insert): ZIO[Conn, MoleculeError, Seq[(Int, Seq[InsertError])]] = {
    sync2zio[Seq[(Int, Seq[InsertError])]]((conn: JdbcConn_JVM) => Spi_sqlite_sync.insert_validate(insert)(conn))
  }
  ...
  override def update_transact(update: Update): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => Spi_sqlite_sync.update_transact(update)(conn)
            case errors                   => throw ValidationErrors(errors)
          }
        )
      )
  ...
  override def update_inspect(update: Update): ZIO[Conn, MoleculeError, String] = {
    sync2zio[String]((conn: JdbcConn_JVM) => Spi_sqlite_sync.update_inspect(update)(conn))
  }

  override def update_validate(update: Update): ZIO[Conn, MoleculeError, Map[String, Seq[String]]] = {
    sync2zio[Map[String, Seq[String]]]((conn: JdbcConn_JVM) => Spi_sqlite_sync.update_validate(update)(conn))
  }
  ...
  override def delete_transact(delete: Delete): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          Spi_sqlite_sync.delete_transact(delete)(conn)
        )
      )
    } yield txReport
  }
  ...
  override def delete_inspect(delete: Delete): ZIO[Conn, MoleculeError, String] = {
    sync2zio[String]((conn: JdbcConn_JVM) => Spi_sqlite_sync.delete_inspect(delete)(conn))
  }
  ...
  override def fallback_rawQuery(
    ...
    for {
      ...
      result <- mapError(ZIO.attemptBlocking(
        Spi_sqlite_sync.fallback_rawQuery(query, debug)(conn)
      ))
    } yield result
  }
  ...
  override def fallback_rawTransact(
    ...
    for {
      ...
      conn = conn0.asInstanceOf[JdbcConn_JVM]
      result <- mapError(ZIO.attemptBlocking(
        Spi_sqlite_sync.fallback_rawTransact(txData, debug)(conn)
      ))
    } yield result
  }
