...
import molecule.db.common.ops.ModelTransformations_.subQueryComparison
...
trait ModelTransformations_ extends ModelUtils {
  ...
  private def getFn(a: AttrOneMan, kw: Kw, n: Option[Int] = None): AggrFn = a match {
    case a: AttrOneManID             => AggrFn("Long", kw.toString, n)
    case a: AttrOneManString         => AggrFn("String", kw.toString, n)
    case a: AttrOneManInt            => AggrFn("Int", kw.toString, n)
    case a: AttrOneManLong           => AggrFn("Long", kw.toString, n)
    case a: AttrOneManFloat          => AggrFn("Float", kw.toString, n)
    case a: AttrOneManDouble         => AggrFn("Double", kw.toString, n)
    case a: AttrOneManBoolean        => AggrFn("Boolean", kw.toString, n)
    case a: AttrOneManBigInt         => AggrFn("BigInt", kw.toString, n)
    case a: AttrOneManBigDecimal     => AggrFn("BigDecimal", kw.toString, n)
    case a: AttrOneManDate           => AggrFn("Date", kw.toString, n)
    case a: AttrOneManDuration       => AggrFn("Duration", kw.toString, n)
    case a: AttrOneManInstant        => AggrFn("Instant", kw.toString, n)
    case a: AttrOneManLocalDate      => AggrFn("LocalDate", kw.toString, n)
    case a: AttrOneManLocalTime      => AggrFn("LocalTime", kw.toString, n)
    case a: AttrOneManLocalDateTime  => AggrFn("LocalDateTime", kw.toString, n)
    case a: AttrOneManOffsetTime     => AggrFn("OffsetTime", kw.toString, n)
    case a: AttrOneManOffsetDateTime => AggrFn("OffsetDateTime", kw.toString, n)
    case a: AttrOneManZonedDateTime  => AggrFn("ZonedDateTime", kw.toString, n)
    case a: AttrOneManUUID           => AggrFn("UUID", kw.toString, n)
    case a: AttrOneManURI            => AggrFn("URI", kw.toString, n)
    case a: AttrOneManByte           => AggrFn("Byte", kw.toString, n)
    case a: AttrOneManShort          => AggrFn("Short", kw.toString, n)
    case a: AttrOneManChar           => AggrFn("Char", kw.toString, n)
  }
  ...
  def addAggrOp[T](dataModel: DataModel, aggrOp: Op, aggrOpV: Option[T]): DataModel = {
    ...
    val last  = es.last match {
      case a: AttrOneMan =>
        val fn = a.op.asInstanceOf[AggrFn]
        a match {
  ...
  def filterAttr(dataModel: DataModel, op: Op, filterAttrMolecule: Molecule): DataModel = {
    val filterAttr = filterAttrMolecule.dataModel.elements.last.asInstanceOf[Attr]
    if (filterAttr.op.isInstanceOf[AggrFn]) {
      // Tacit aggregation treated as subquery
      subQueryComparison(dataModel, op, filterAttrMolecule)
    } else {
      resolveFilterAttr(dataModel: DataModel, op: Op, filterAttrMolecule: Molecule, filterAttr)
    }
  }

  private def resolveFilterAttr(dataModel: DataModel, op: Op, filterAttrMolecule: Molecule, filterAttr: Attr): DataModel = {
    val es    = dataModel.elements
    val attrs = es.last match {
      case a: Attr =>
        ...
        val adjacent           = if (a.ent == filterAttr.ent) List(filterAttr) else Nil
        ...
        val attrWithFilterAttr = a match {
          case a: AttrOne => a match {
            case a: AttrOneMan => a match {
              ...
              case a: AttrOneManID             => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManString         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManInt            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManLong           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManFloat          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManDouble         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManBoolean        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManBigInt         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManBigDecimal     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManDate           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManDuration       => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManInstant        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManLocalDate      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManLocalTime      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManLocalDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManOffsetTime     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManOffsetDateTime => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManZonedDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManUUID           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManURI            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManByte           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManShort          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneManChar           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
            }
            case a: AttrOneTac => a match {
              case a: AttrOneTacID             => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacString         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacInt            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacLong           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacFloat          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacDouble         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacBoolean        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacBigInt         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacBigDecimal     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacDate           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacDuration       => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacInstant        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacLocalDate      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacLocalTime      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacLocalDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacOffsetTime     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacOffsetDateTime => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacZonedDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacUUID           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacURI            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacByte           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacShort          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrOneTacChar           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
            }
            case a             => unexpected(a)
          }
          ...
          case a: AttrSet => a match {
            case a: AttrSetMan => a match {
              case a: AttrSetManID             => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManString         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManInt            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManLong           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManFloat          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManDouble         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManBoolean        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManBigInt         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManBigDecimal     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManDate           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManDuration       => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManInstant        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManLocalDate      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManLocalTime      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManLocalDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManOffsetTime     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManOffsetDateTime => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManZonedDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManUUID           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManURI            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManByte           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManShort          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetManChar           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
            }
            case a: AttrSetTac => a match {
              case a: AttrSetTacID             => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacString         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacInt            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacLong           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacFloat          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacDouble         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacBoolean        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacBigInt         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacBigDecimal     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacDate           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacDuration       => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacInstant        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacLocalDate      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacLocalTime      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacLocalDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacOffsetTime     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacOffsetDateTime => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacZonedDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacUUID           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacURI            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacByte           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacShort          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSetTacChar           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
            }
            case a             => unexpected(a)
          }
          ...
          case a: AttrSeq => a match {
            case a: AttrSeqMan => a match {
              case a: AttrSeqManID             => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManString         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManInt            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManLong           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManFloat          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManDouble         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManBoolean        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManBigInt         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManBigDecimal     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManDate           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManDuration       => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManInstant        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManLocalDate      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManLocalTime      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManLocalDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManOffsetTime     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManOffsetDateTime => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManZonedDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManUUID           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManURI            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManByte           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManShort          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqManChar           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
            }
            case a: AttrSeqTac => a match {
              case a: AttrSeqTacID             => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacString         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacInt            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacLong           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacFloat          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacDouble         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacBoolean        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacBigInt         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacBigDecimal     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacDate           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacDuration       => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacInstant        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacLocalDate      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacLocalTime      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacLocalDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacOffsetTime     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacOffsetDateTime => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacZonedDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacUUID           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacURI            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacByte           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacShort          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrSeqTacChar           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
            }
            case a             => unexpected(a)
          }
          ...
          case a: AttrMap => a match {
            case a: AttrMapMan => a match {
              case a: AttrMapManID             => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManString         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManInt            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManLong           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManFloat          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManDouble         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManBoolean        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManBigInt         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManBigDecimal     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManDate           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManDuration       => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManInstant        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManLocalDate      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManLocalTime      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManLocalDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManOffsetTime     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManOffsetDateTime => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManZonedDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManUUID           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManURI            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManByte           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManShort          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapManChar           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
            }
            case a: AttrMapTac => a match {
              case a: AttrMapTacID             => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacString         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacInt            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacLong           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacFloat          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacDouble         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacBoolean        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacBigInt         => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacBigDecimal     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacDate           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacDuration       => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacInstant        => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacLocalDate      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacLocalTime      => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacLocalDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacOffsetTime     => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacOffsetDateTime => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacZonedDateTime  => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacUUID           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacURI            => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacByte           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacShort          => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
              case a: AttrMapTacChar           => a.copy(op = op, filterAttr = Some((-2, filterPath, filterAttr)))
            }
            case a             => unexpected(a)
          }
        }
  ...
  def subQueryComparison(dataModel: DataModel, op: Op, subQueryMolecule: Molecule): DataModel = {
    ...
    val attrWithSubQuery = es.last match {
      case a: AttrOneMan => a match {
        case a: AttrOneManID             => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManString         => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManInt            => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManLong           => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManFloat          => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManDouble         => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManBoolean        => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManBigInt         => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManBigDecimal     => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManDate           => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManDuration       => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManInstant        => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManLocalDate      => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManLocalTime      => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManLocalDateTime  => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManOffsetTime     => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManOffsetDateTime => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManZonedDateTime  => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManUUID           => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManURI            => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManByte           => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManShort          => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneManChar           => a.copy(op = op, subquery = Some(subQuery))
      }
      case a: AttrOneTac => a match {
        case a: AttrOneTacID             => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacString         => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacInt            => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacLong           => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacFloat          => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacDouble         => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacBoolean        => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacBigInt         => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacBigDecimal     => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacDate           => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacDuration       => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacInstant        => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacLocalDate      => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacLocalTime      => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacLocalDateTime  => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacOffsetTime     => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacOffsetDateTime => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacZonedDateTime  => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacUUID           => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacURI            => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacByte           => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacShort          => a.copy(op = op, subquery = Some(subQuery))
        case a: AttrOneTacChar           => a.copy(op = op, subquery = Some(subQuery))
      }
      case other         => unexpected(other)
    }
    dataModel.copy(elements = es.init :+ attrWithSubQuery)
  }

  //  def reverseTopLevelSorting(es: List[Element]): List[Element] = {
  //    es.map {
  //      case attr: AttrOneMan => attr match {
  //        case a@AttrOneManID(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)             => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManString(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)         => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManInt(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)            => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManLong(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManFloat(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)          => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManDouble(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)         => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManBoolean(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)        => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManBigInt(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)         => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManBigDecimal(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)     => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManDate(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManDuration(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)       => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManInstant(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)        => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManLocalDate(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)      => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManLocalTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)      => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManLocalDateTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)  => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManOffsetTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)     => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManOffsetDateTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _) => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManZonedDateTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)  => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManUUID(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManURI(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)            => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManByte(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManShort(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)          => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneManChar(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a                                                                          => a
  //      }
  //      case attr: AttrOneOpt => attr match {
  //        case a@AttrOneOptID(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)             => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptString(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)         => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptInt(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)            => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptLong(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptFloat(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)          => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptDouble(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)         => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptBoolean(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)        => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptBigInt(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)         => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptBigDecimal(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)     => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptDate(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptDuration(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)       => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptInstant(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)        => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptLocalDate(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)      => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptLocalTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)      => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptLocalDateTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)  => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptOffsetTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)     => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptOffsetDateTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _) => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptZonedDateTime(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)  => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptUUID(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptURI(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)            => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptByte(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptShort(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)          => a.copy(sort = Some(reverseSort(sort)))
  //        case a@AttrOneOptChar(_, _, _, _, _, _, _, _, _, _, Some(sort), _, _)           => a.copy(sort = Some(reverseSort(sort)))
  //        case a                                                                          => a
  //      }
  //      case other            => other
  //    }
