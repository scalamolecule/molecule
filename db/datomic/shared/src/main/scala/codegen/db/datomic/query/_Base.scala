...
object _Base extends DatomicGenBase("Base", "/query") {
  ...
  case class Chunk(i: Int) extends TemplateVals(i) {
    val resolvers = (1 to i).map { j => s"val t$j = typers(${j - 1})" }.mkString("\n    ")
    val tuple     = (1 to i).map { j => s"t$j(row.get(${j - 1}))" }.mkString(",\n        ")
    val body      =
      s"""
