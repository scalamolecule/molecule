...
object ResolveOption extends DatomicTestSuite {
...
//      val pairs               = (1 to 5).toList.map((Random.between(1, 3), _))
      val pairs               = List(
        (Random.between(1, 3), Some(1)),
        (Random.between(1, 3), Some(2)),
        (Random.between(1, 3), Some(3)),
        (Random.between(1, 3), None),
        (Random.between(1, 3), None),
      )
      ...
      val query               = (cursor: String, limit: Int) => Unique.i.a1.int_?.a2.query.from(cursor).limit(limit)
      for {
