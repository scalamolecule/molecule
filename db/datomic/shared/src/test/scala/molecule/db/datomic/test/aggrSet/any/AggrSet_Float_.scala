...
object AggrSet_Float_ extends DatomicTestSuite {
  ...
  lazy val tests = Tests {
    ...
    "min" - types { implicit conn =>
      ...
      Ns.n.floats(min).query.get ==> List(
        (1, Set(float1)),
        (2, Set(float2)),
      )
      // Same as
      Ns.n.floats(min(1)).query.get ==> List(
        (1, Set(float1)),
        (2, Set(float2)),
      )

      Ns.n.floats(min(2)).query.get ==> List(
        (1, Set(float1, float2)),
        (2, Set(float2, float3)),
      )
    }
    ...
    "max" - types { implicit futConn =>
      ...
      Ns.n.floats(max).query.get ==> List(
        (1, Set(float2)),
        (2, Set(float4)),
      )
      // Same as
      Ns.n.floats(max(1)).query.get ==> List(
        (1, Set(float2)),
        (2, Set(float4)),
      )

      Ns.n.floats(max(2)).query.get ==> List(
        (1, Set(float1, float2)),
        (2, Set(float3, float4)),
      )
    }
    ...
    "count, countDistinct" - types { implicit conn =>
      ...
      Ns.n.a1.floats(count).query.get ==> List(
        (1, 2),
        (2, 6)
      )
      Ns.n.a1.floats(countDistinct).query.get ==> List(
        (1, 2),
        (2, 3)
      )
    }
