...
object AggrSet_Byte_ extends DatomicTestSuite {
  ...
  lazy val tests = Tests {
    ...
    "min" - types { implicit conn =>
      ...
      Ns.n.bytes(min).query.get ==> List(
        (1, Set(byte1)),
        (2, Set(byte2)),
      )
      // Same as
      Ns.n.bytes(min(1)).query.get ==> List(
        (1, Set(byte1)),
        (2, Set(byte2)),
      )

      Ns.n.bytes(min(2)).query.get ==> List(
        (1, Set(byte1, byte2)),
        (2, Set(byte2, byte3)),
      )
    }
    ...
    "max" - types { implicit futConn =>
      ...
      Ns.n.bytes(max).query.get ==> List(
        (1, Set(byte2)),
        (2, Set(byte4)),
      )
      // Same as
      Ns.n.bytes(max(1)).query.get ==> List(
        (1, Set(byte2)),
        (2, Set(byte4)),
      )

      Ns.n.bytes(max(2)).query.get ==> List(
        (1, Set(byte1, byte2)),
        (2, Set(byte3, byte4)),
      )
    }
    ...
    "count, countDistinct" - types { implicit conn =>
      ...
      Ns.n.a1.bytes(count).query.get ==> List(
        (1, 2),
        (2, 6)
      )
      Ns.n.a1.bytes(countDistinct).query.get ==> List(
        (1, 2),
        (2, 3)
      )
    }
