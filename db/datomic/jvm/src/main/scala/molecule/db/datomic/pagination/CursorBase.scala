...
//import java.net.URI
//import java.util
//import java.util.{Base64, Date, UUID, Collection => jCollection, List => jList}
...
//
//
//  final protected def getFlatCursorIndex(
//    conn: Connection,
...
//    }
//    val otherIndex = collectSameSortValues(initIndex, rows.get(initIndex))
//    (otherIndex, curWindow)
//  }
//
...
//    val model = getCursorModelWindow(lastSortValues)
...
//    val model = getCursorModelWindow(firstSortValues)
//    val query = Model2Query(model).get._1
//    conn.usingAdhocDbView(AsOf(TxLong(t))).jvmQuery(model, query).map { prevWindowUnsorted =>
//      val prevWindow = new util.ArrayList(prevWindowUnsorted)
//      prevWindow.sort(this)
...
//        case (true, _)  => getFlatCursorIndex(
...
//  }
//
//  final protected def getCursorModel(
//    forward: Boolean,
//    firstSortValues: List[String],
//    lastSortValues: List[String]
//  ): Model = {
...
//  }
//
//  final protected def getCursorModelWindow(sortValues: List[String]): Model = {
...
//
//      val model = getCursorModel(forward, firstSortValues, lastSortValues)
//      val query = Model2Query(model).get._1
