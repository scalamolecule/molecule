  ...
  protected def exprAttr(es: List[Element], op: Op, attrMolecule: Molecule[_]): List[Element] = {
    ...
    val attr     = es.last match {
      case a: AttrOne => a match {
        case a: AttrOneMan => a match {
          case a: AttrOneManString     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManInt        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManLong       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManFloat      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManDouble     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManBoolean    => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManBigInt     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManBigDecimal => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManDate       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManUUID       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManURI        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManByte       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManShort      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneManChar       => a.copy(op = op, exprAttr = Some(exprAttr))
        }
        case a: AttrOneOpt => a match {
          case a: AttrOneOptString     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptInt        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptLong       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptFloat      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptDouble     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptBoolean    => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptBigInt     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptBigDecimal => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptDate       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptUUID       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptURI        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptByte       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptShort      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneOptChar       => a.copy(op = op, exprAttr = Some(exprAttr))
        }
        case a: AttrOneTac => a match {
          case a: AttrOneTacString     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacInt        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacLong       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacFloat      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacDouble     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacBoolean    => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacBigInt     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacBigDecimal => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacDate       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacUUID       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacURI        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacByte       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacShort      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrOneTacChar       => a.copy(op = op, exprAttr = Some(exprAttr))
        }
      }
      case a: AttrSet => a match {
        case a: AttrSetMan => a match {
          case a: AttrSetManString     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManInt        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManLong       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManFloat      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManDouble     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManBoolean    => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManBigInt     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManBigDecimal => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManDate       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManUUID       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManURI        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManByte       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManShort      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetManChar       => a.copy(op = op, exprAttr = Some(exprAttr))
        }
        case a: AttrSetOpt => a match {
          case a: AttrSetOptString     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptInt        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptLong       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptFloat      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptDouble     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptBoolean    => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptBigInt     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptBigDecimal => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptDate       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptUUID       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptURI        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptByte       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptShort      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetOptChar       => a.copy(op = op, exprAttr = Some(exprAttr))
        }
        case a: AttrSetTac => a match {
          case a: AttrSetTacString     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacInt        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacLong       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacFloat      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacDouble     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacBoolean    => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacBigInt     => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacBigDecimal => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacDate       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacUUID       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacURI        => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacByte       => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacShort      => a.copy(op = op, exprAttr = Some(exprAttr))
          case a: AttrSetTacChar       => a.copy(op = op, exprAttr = Some(exprAttr))
        }
      }
      case a          => unexpected(a)
    }
