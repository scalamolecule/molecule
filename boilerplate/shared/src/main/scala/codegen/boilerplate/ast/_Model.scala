          ...
          case "Opt" =>
            if (format_?)
              s"""def format(v: $baseType): String = $format
                 ...
                 |      s\"\"\"$attrType("$$ns", "$$attr", $$op, $$vss, $${o(validator)}, $$errs, $$vats, $${oStr(status)}, $${oStr(sort)})\"\"\"""".stripMargin
            else
              s"""def vss: String = vs.fold("None")(_.mkString("Some(Seq(", ", ", "))"))
                 |      s\"\"\"$attrType("$$ns", "$$attr", $$op, $$vss, $${opt(validator)}, $$errs, $$vats, $${oStr(status)}, $${oStr(sort)})\"\"\"""".stripMargin
          case _     =>
            if (format_?)
              s"""def format(v: $baseType): String = $format
