  ...
  case class Trait(arity: Int) extends TemplateVals(arity) {
    val body =
      s"""
         ...
         |  def add    (vs   : Seq[t]                )               : Ns[${`A..V`}, t] = _exprSetMan(Add    , Seq(vs.toSet)             )
         |  def replace(ab   : (t, t), abs: (t, t)*  )               : Ns[${`A..V`}, t] = _exprSetMan(Replace, abs2sets(ab +: abs)       )
         |  def replace(abs  : Seq[(t, t)]           )               : Ns[${`A..V`}, t] = _exprSetMan(Replace, abs2sets(abs)             )
         |  def delete (v    : t, vs: t*             )               : Ns[${`A..V`}, t] = _exprSetMan(Delete , Seq((v +: vs).toSet)      )
         |  def delete (vs   : Seq[t]                )               : Ns[${`A..V`}, t] = _exprSetMan(Delete , Seq(vs.toSet)             )
         |}""".stripMargin
  }
}
