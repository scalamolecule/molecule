  ...
  case class Trait(arity: Int) extends TemplateVals(arity) {
    val body =
      s"""
         ...
         |  def add   (vs   : Iterable[t]          )               : Ns[${`A..V, `}t] = _exprSetTac(Add    , Seq(vs.toSet)             )
         |  def swap  (ab   : (t, t), abs: (t, t)* )               : Ns[${`A..V, `}t] = _exprSetTac(Swap   , abs2sets(ab +: abs)       )
         |  def swap  (abs  : Seq[(t, t)]          )               : Ns[${`A..V, `}t] = _exprSetTac(Swap   , abs2sets(abs)             )
         |  def remove(v    : t, vs: t*            )               : Ns[${`A..V, `}t] = _exprSetTac(Remove , Seq((v +: vs).toSet)      )
         |  def remove(vs   : Iterable[t]          )               : Ns[${`A..V, `}t] = _exprSetTac(Remove , Seq(vs.toSet)             )
         |}""".stripMargin
  }
}
