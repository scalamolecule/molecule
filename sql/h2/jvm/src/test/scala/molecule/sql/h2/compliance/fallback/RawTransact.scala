...
object Test_RawTransact extends TestSuite_h2 {
  ...
  override lazy val tests = Tests {
    ...
    "Insert Set" - types { implicit conn =>
      for {
        _ <- rawTransact(s"insert into Ns (stringSet        ) values (array['$string1', '$string2'])")
        _ <- rawTransact(s"insert into Ns (intSet           ) values (array[$int1, $int2])")
        _ <- rawTransact(s"insert into Ns (longSet          ) values (array[$long1, $long2])")
        _ <- rawTransact(s"insert into Ns (floatSet         ) values (array[$float1, $float2])")
        _ <- rawTransact(s"insert into Ns (doubleSet        ) values (array[$double1, $double2])")
        _ <- rawTransact(s"insert into Ns (booleanSet       ) values (array[$boolean1, $boolean2])")
        _ <- rawTransact(s"insert into Ns (bigIntSet        ) values (array[$bigInt1, $bigInt2])")
        _ <- rawTransact(s"insert into Ns (bigDecimalSet    ) values (array[$bigDecimal1, $bigDecimal2])")
        _ <- rawTransact(s"insert into Ns (dateSet          ) values (array[${date1.getTime}, ${date2.getTime}])")
        _ <- rawTransact(s"insert into Ns (durationSet      ) values (array['${duration1.toString}', '${duration2.toString}'])")
        _ <- rawTransact(s"insert into Ns (instantSet       ) values (array['${instant1.toString}', '${instant2.toString}'])")
        _ <- rawTransact(s"insert into Ns (localDateSet     ) values (array['${localDate1.toString}', '${localDate2.toString}'])")
        _ <- rawTransact(s"insert into Ns (localTimeSet     ) values (array['${localTime1.toString}', '${localTime2.toString}'])")
        _ <- rawTransact(s"insert into Ns (localDateTimeSet ) values (array['${localDateTime1.toString}', '${localDateTime2.toString}'])")
        _ <- rawTransact(s"insert into Ns (offsetTimeSet    ) values (array['${offsetTime1.toString}', '${offsetTime2.toString}'])")
        _ <- rawTransact(s"insert into Ns (offsetDateTimeSet) values (array['${offsetDateTime1.toString}', '${offsetDateTime2.toString}'])")
        _ <- rawTransact(s"insert into Ns (zonedDateTimeSet ) values (array['${zonedDateTime1.toString}', '${zonedDateTime2.toString}'])")
        _ <- rawTransact(s"insert into Ns (uuidSet          ) values (array['$uuid1', '$uuid2'])")
        _ <- rawTransact(s"insert into Ns (uriSet           ) values (array['$uri1', '$uri2'])")
        _ <- rawTransact(s"insert into Ns (byteSet          ) values (array[$byte1, $byte2])")
        _ <- rawTransact(s"insert into Ns (shortSet         ) values (array[$short1, $short2])")
        _ <- rawTransact(s"insert into Ns (charSet          ) values (array['$char1', '$char2'])")
