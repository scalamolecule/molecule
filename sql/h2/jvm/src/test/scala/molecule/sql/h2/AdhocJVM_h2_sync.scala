...
import molecule.coreTests.dataModels.schema.TypesSchema
  ...
  override lazy val tests = Tests {
    ...
    "unitOfWork" - {
      ...
      "money transfer2" - types { implicit conn =>
        // Initial balance in two bank accounts
        Ns.int(100).save.transact
        Ns.int(100).query.get ==> List(100)

        try {
          unitOfWork {
            Ns.int_(100).delete.transact
            Ns.int(100).query.get ==> List()
            throw new Exception(
              "Insufficient funds in fromAccount..."
            )
          }
      ...
      "money transfer3" - types { implicit conn =>
        // Initial balance in two bank accounts
        Ns.s("fromAccount").int(100).save.transact
        Ns.s("toAccount").int(50).save.transact
