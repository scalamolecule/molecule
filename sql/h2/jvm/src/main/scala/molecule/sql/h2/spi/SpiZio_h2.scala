...
trait SpiZio_h2 extends SpiZio with SpiZioBase_h2 with ModelUtils {
  ...
  override def save_transact(save: Save): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => SpiSync_h2.save_transact(
  ...
  override def insert_transact(insert: Insert): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => SpiSync_h2.insert_transact(
  ...
  override def update_transact(update: Update): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => SpiSync_h2.update_transact(
