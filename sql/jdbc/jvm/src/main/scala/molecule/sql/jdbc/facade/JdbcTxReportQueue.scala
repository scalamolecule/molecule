 ...
 */
case class JdbcTxReportQueue(javaQueue: jBlockingQueue[jMap[_, _]]) extends JavaConversions {
  ...
  def drain: List[SqlTxReport] = {
    ...
    txMaps.forEach(txMap => list += MakeJdbcTxReport(txMap))
    list.result()
  }
  ...
  def poll(timeout: Duration): Option[SqlTxReport] =
    Option(javaQueue.poll(timeout.toNanos, NANOSECONDS)).map(MakeJdbcTxReport(_))
   ...
   */
  def poll: Option[SqlTxReport] = Option(javaQueue.poll()).map(MakeJdbcTxReport(_))
   ...
   */
  def take: SqlTxReport = MakeJdbcTxReport(javaQueue.take())
   ...
   *         queue is empty.
   */
  def peek: Option[SqlTxReport] = Option(javaQueue.peek()).map(MakeJdbcTxReport(_))
  ...
  def iterator: Iterator[SqlTxReport] =
    javaQueue.iterator.asScala.map(MakeJdbcTxReport(_))
