...
trait JdbcApiZio extends JdbcZioApiBase with ApiZio with FutureUtils {
  ...
  private def getResult[T](query: JdbcConn_JS => Future[T]): ZIO[Connection, MoleculeError, T] = {
    for {
      conn0 <- ZIO.service[Connection]
      conn = conn0.asInstanceOf[JdbcConn_JS]
