...
trait SqlHandler {
  ...
  def connect(proxy: SqlProxy, url: String): SqlConn_JVM = blocking {
    val sqlConn = java.sql.DriverManager.getConnection(url)
    SqlConn_JVM(proxy, sqlConn)
  }

  def recreateDb(proxy: SqlProxy, url: String): SqlConn_JVM = {
    //    val stmts = proxy.schema.head
    val stmts                    =
      """create table Ns(
        |    e long auto_increment primary key,
        |    int int not null
        |);
        |""".stripMargin
