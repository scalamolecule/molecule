...
import molecule.sql.core.util.MakeSqlTxReport
 ...
 */
case class SqlTxReportQueue(javaQueue: jBlockingQueue[jMap[_, _]]) extends JavaConversions {
   ...
   */
  def drain: List[SqlTxReport] = {
    val txMaps = new jLinkedList[jMap[_, _]]
    javaQueue.drainTo(txMaps)
    val list = List.newBuilder[SqlTxReport]
    txMaps.forEach(txMap => list += MakeSqlTxReport(txMap))
    list.result()
  }
   ...
   * @return a list of all available tranaction reports.
   */
  def drain(maxReports: Int): List[SqlTxReport] = {
    val txMaps = new jLinkedList[jMap[_, _]]
   ...
   */
  def poll(timeout: Duration): Option[SqlTxReport] =
    Option(javaQueue.poll(timeout.toNanos, NANOSECONDS)).map(MakeSqlTxReport(_))
   ...
   */
  def poll: Option[SqlTxReport] = Option(javaQueue.poll()).map(MakeSqlTxReport(_))
   ...
   */
  def take: SqlTxReport = MakeSqlTxReport(javaQueue.take())
   ...
   *         queue is empty.
   */
  def peek: Option[SqlTxReport] = Option(javaQueue.peek()).map(MakeSqlTxReport(_))
   ...
   *         in this queue.
   */
  def iterator: Iterator[SqlTxReport] =
    javaQueue.iterator.asScala.map(MakeSqlTxReport(_))
