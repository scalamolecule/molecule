...
trait SqlTxBase_JVM extends SqlDataType_JVM with ModelUtils {
  ...
  // todo: real solution
  protected val connectionPool = mutable.HashMap.empty[UUID, Future[SqlConn_JVM]]
  ...
  protected def getConn(proxy: ConnProxy): Future[SqlConn_JVM] = {
  ...
  protected def getFreshConn(proxy: ConnProxy): Future[SqlConn_JVM] = {
    proxy match {
