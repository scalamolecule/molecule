...
import molecule.sql.core.query.cursorStrategy.{CursorUtils, NoUnique, PrimaryUnique, SubUnique}
...
case class SqlQueryResolveCursor[Tpl](
  ...
  def getListFromCursor_async(implicit conn: SqlConn_JVM, ec: ExecutionContext)
  : Future[(List[Tpl], String, Boolean)] = future(getListFromCursor_sync)


  def getListFromCursor_sync(implicit conn: SqlConn_JVM)
  ...
  private def getInitialPage(limit: Int)(implicit conn: SqlConn_JVM)
  : (List[Tpl], String, Boolean) = {
    val forward     = limit > 0
  ...
  private def initialCursor(conn: SqlConn_JVM, tpls: List[Tpl]): String = {
