...
class RawTransact extends MUnitSuite with DbProviders_sqlite with TestUtils {
  ...
  "Insert" - types { implicit conn =>
    for {
      ...
      _ <- rawTransact(s"insert into Entity (localTime     ) values ('${localTime1.toString}')")
  ...
  "Update" - types { implicit conn =>
    for {
      ...
      _ <- rawTransact(s"update Entity set localTime      = '${localTime2.toString}'      where id = $id13")
