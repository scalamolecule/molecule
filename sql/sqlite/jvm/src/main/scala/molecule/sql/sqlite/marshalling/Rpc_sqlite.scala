...
object Rpc_sqlite
  ...
  override def save(
    ...
    for {
      conn <- getConn(proxy)
      data = new ResolveSave with Save_sqlite {
  ...
  override def insert(
    ...
    for {
      ...
      data = new ResolveInsert with Insert_sqlite {
        override lazy val sqlConn: Connection = conn.sqlConn
  ...
  override def update(
    ...
    for {
      ...
      txReport <- if (isRefUpdate(elements)) {
        ...
        val data = new ResolveUpdate(conn.proxy, isUpsert) with Update_sqlite {
          override lazy val sqlConn: Connection = conn.sqlConn
  ...
  override def delete(
    ...
    for {
      conn <- getConn(proxy)
      data = new ResolveDelete with Delete_sqlite {
        override lazy val sqlConn: Connection = conn.sqlConn
