...
trait LambdasOne_sqlite extends LambdasOne with BigEncoding { self: SqlQueryBase =>
...
//  override lazy val sql2oneBigInt    : (RS, Int) => BigInt     = (row: RS, paramIndex: Int) => decodeBigInt(row.getString(paramIndex))
  override lazy val sql2oneBigInt    : (RS, Int) => BigInt     = (row: RS, paramIndex: Int) => BigInt(row.getString(paramIndex).replace(".0", ""))
