...
object RawTransact extends TestSuite_mariadb {
  ...
  override lazy val tests = Tests {
    ...
    "Insert Set" - types { implicit conn =>
      for {
        _ <- Ns.strings(Set("a", "b")).save.inspect
        _ <- rawTransact(s"""insert into Ns (strings        ) values ('["$string1", "$string2"]')""")
        _ <- rawTransact(s"""insert into Ns (ints           ) values ('[$int1, $int2]')""")
        _ <- rawTransact(s"""insert into Ns (longs          ) values ('[$long1, $long2]')""")
        _ <- rawTransact(s"""insert into Ns (floats         ) values ('[$float1, $float2]')""")
        _ <- rawTransact(s"""insert into Ns (doubles        ) values ('[$double1, $double2]')""")
        _ <- rawTransact(s"""insert into Ns (booleans       ) values ('[$boolean1, $boolean2]')""")
        _ <- rawTransact(s"""insert into Ns (bigInts        ) values ('[$bigInt1, $bigInt2]')""")
        _ <- rawTransact(s"""insert into Ns (bigDecimals    ) values ('[$bigDecimal1, $bigDecimal2]')""")
        _ <- rawTransact(s"""insert into Ns (dates          ) values ('[${date1.getTime}, ${date2.getTime}]')""")
        _ <- rawTransact(s"""insert into Ns (durations      ) values ('["${duration1.toString}", "${duration2.toString}"]')""")
        _ <- rawTransact(s"""insert into Ns (instants       ) values ('["${instant1.toString}", "${instant2.toString}"]')""")
        _ <- rawTransact(s"""insert into Ns (localDates     ) values ('["${localDate1.toString}", "${localDate2.toString}"]')""")
        _ <- rawTransact(s"""insert into Ns (localTimes     ) values ('["${localTime1.toString}", "${localTime2.toString}"]')""")
        _ <- rawTransact(s"""insert into Ns (localDateTimes ) values ('["${localDateTime1.toString}", "${localDateTime2.toString}"]')""")
        _ <- rawTransact(s"""insert into Ns (offsetTimes    ) values ('["${offsetTime1.toString}", "${offsetTime2.toString}"]')""")
        _ <- rawTransact(s"""insert into Ns (offsetDateTimes) values ('["${offsetDateTime1.toString}", "${offsetDateTime2.toString}"]')""")
        _ <- rawTransact(s"""insert into Ns (zonedDateTimes ) values ('["${zonedDateTime1.toString}", "${zonedDateTime2.toString}"]')""")
        _ <- rawTransact(s"""insert into Ns (uuids          ) values ('["$uuid1", "$uuid2"]')""")
        _ <- rawTransact(s"""insert into Ns (uris           ) values ('["$uri1", "$uri2"]')""")
        _ <- rawTransact(s"""insert into Ns (bytes          ) values ('[$byte1, $byte2]')""")
        _ <- rawTransact(s"""insert into Ns (shorts         ) values ('[$short1, $short2]')""")
        _ <- rawTransact(s"""insert into Ns (chars          ) values ('["$char1", "$char2"]')""")
