...
class RawTransact extends MUnitSuite with DbProviders_mysql with TestUtils {
  ...
  "Semantics" - types { implicit conn =>
    for {
      ...
      // Insert 2 rows
      _ <- rawTransact("insert into Entity (i, boolean_) values (1, true), (2, false)")
      // Rows saved
  ...
  "Insert" - types { implicit conn =>
    for {
      ...
      _ <- rawTransact(s"insert into Entity (boolean_      ) values ($boolean1)")
      ...
      _ <- rawTransact(s"insert into Entity (byte_         ) values ($byte1)")
  ...
  "Update" - types { implicit conn =>
    for {
      ...
      // Update all values
      _ <- rawTransact(s"update Entity set string_        = '$string2'                    where id = $id1")
      ...
      _ <- rawTransact(s"update Entity set boolean_       = $boolean2                     where id = $id6")
      ...
      _ <- rawTransact(s"update Entity set byte_          = $byte2                        where id = $id20")
