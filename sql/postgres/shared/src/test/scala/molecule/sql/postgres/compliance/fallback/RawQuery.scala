  ...
  override lazy val tests = Tests {
    ...
    "Card Set types" - types { implicit conn =>
      ...
      for {
        ...
        _ <- rawQuery(q("floats")).map(_.head ==> List(Set(new jBigDecimal(float1.toString))))
        _ <- rawQuery(q("doubles")).map(_.head ==> List(Set(double1)))
        _ <- rawQuery(q("booleans")).map(_.head ==> List(Set(boolean1)))
        _ <- rawQuery(q("bigInts")).map(_.head ==> List(Set(new jBigDecimal(bigInt1.bigInteger))))
        _ <- rawQuery(q("bigDecimals")).map(_.head ==> List(Set(bigDecimal1.bigDecimal)))
        _ <- rawQuery(q("dates")).map(_.head ==> List(Set(date1.getTime)))
