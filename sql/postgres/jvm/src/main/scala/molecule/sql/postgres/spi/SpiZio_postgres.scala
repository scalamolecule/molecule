...
trait SpiZio_postgres extends SpiZio with SpiZioBase_postgres with ModelUtils {
  ...
  override def save_transact(save: Save): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => SpiSync_postgres.save_transact(save)(conn)
  ...
  override def insert_transact(insert: Insert): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => SpiSync_postgres.insert_transact(insert)(conn)
  ...
  override def update_transact(update: Update): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          errors match {
            case errors if errors.isEmpty => SpiSync_postgres.update_transact(update)(conn)
            case errors                   => throw ValidationErrors(errors)
          }
        )
      )
  ...
  override def delete_transact(delete: Delete): ZIO[Conn, MoleculeError, TxReport] = {
    for {
      ...
      txReport <- mapError(
        ZIO.attemptBlocking(
          SpiSync_postgres.delete_transact(delete)(conn)
        )
      )
    } yield txReport
  }
